...
export type ExpmapOutput = { encoding?: VideoEncoding; width: number; height: number; fps: number; codec: Mp4Codec | 'auto'; maxSamples: number; sampleDistribution: ExpmapSampleDistribution }
export const DEFAULT_EXPMAP_OUTPUT: ExpmapOutput = { encoding: { ...DEFAULT_VIDEO_ENCODING }, width: 3840, height: 2160, fps: 60, codec: 'auto', maxSamples: 16, sampleDistribution: 'grid' }
export function loadExpmapOutput(): ExpmapOutput {
  try {
    ...
    return {
      encoding: normalizeVideoEncoding(v.encoding),
