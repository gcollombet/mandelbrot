...
export async function saveTextureEntry(
  ...
  const existing: TextureMetadata | undefined = await reqToPromise(store.index('guid').get(guid));
  if (existing && existing.name !== resolvedName) {
    store.delete(existing.name);
    transaction.objectStore(BLOB_STORE_NAME).delete(existing.name);
  }
  store.put(metadata);
...
export async function renameTextureEntry(
  ...
  await saveTextureEntry(newName, blob, metadata.thumbnail, metadata.date, metadata.guid, metadata.favorite ?? false, metadata.remote, metadata, metadata);
}
...
export async function getTextureCacheSnapshot(guid: string): Promise<{record: TextureMetadata; blob: Blob} | null> {
  const db = await openDB();
  const transaction = db.transaction([METADATA_STORE_NAME, BLOB_STORE_NAME], 'readonly');
  const done = transactionDone(transaction);
  const record: TextureMetadata | undefined = await reqToPromise(transaction.objectStore(METADATA_STORE_NAME).index('guid').get(guid));
  const data = record ? await reqToPromise<{blob: Blob} | undefined>(transaction.objectStore(BLOB_STORE_NAME).get(record.name)) : undefined;
  await done;
...
export async function applyCloudTextureEntry(metadata: TextureMetadata, blob: Blob, revision: number, expected?: ScopedCacheFields | null): Promise<void> {
  ...
  const metadataStore = transaction.objectStore(METADATA_STORE_NAME);
  const blobStore = transaction.objectStore(BLOB_STORE_NAME);
  const existing: TextureMetadata | undefined = await reqToPromise(metadataStore.index('guid').get(metadata.guid));
...
export async function acknowledgeTextureEntry(
  guid: string, revision: number, details: TextureStorageDetails = {}, expected?: ScopedCacheFields, blob?: Blob,
): Promise<void> {
  const db = await openDB();
  const transaction = db.transaction([METADATA_STORE_NAME, BLOB_STORE_NAME], 'readwrite');
  const done = transactionDone(transaction);
  ...
  if (metadata && matchesCacheSnapshot(metadata, expected)) {
    store.put({...metadata, ...details, unavailable: false, ...syncedPersonalCacheFields(metadata, revision)});
    if (blob) transaction.objectStore(BLOB_STORE_NAME).put({name: metadata.name, blob});
  }
  await done;
}
...
export async function purgeTextureEntryByGuid(guid: string, expected?: ScopedCacheFields): Promise<void> {
  const db = await openDB();
  const transaction = db.transaction([METADATA_STORE_NAME, BLOB_STORE_NAME], 'readwrite');
  const done = transactionDone(transaction);
  const store = transaction.objectStore(METADATA_STORE_NAME);
