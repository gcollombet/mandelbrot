...
export type VideoExportPreferences = {
  ...
  hdrExposure?: number
  /** HDR constant-quality quantizer (0 = best, 63 = worst). */
  hdrQuantizer?: number
  codec: Mp4Codec | 'auto'
  motion: ExpmapMotion
  filename: string
...
export function normalizeVideoExportPreferences(value: unknown): VideoExportPreferences {
  ...
  return {
    ...
    hdrQuantizer: typeof raw.hdrQuantizer === 'number' && Number.isInteger(raw.hdrQuantizer) && raw.hdrQuantizer >= 0 && raw.hdrQuantizer <= 63 ? raw.hdrQuantizer : d.hdrQuantizer,
