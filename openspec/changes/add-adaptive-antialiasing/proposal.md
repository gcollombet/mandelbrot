...
- Add **idle-time antialiasing**: a public `Engine.triggerAaAccumulation()` (wired to a UI button/shortcut) starts accumulating extra full renders, each sampled at a finite-prefix-balanced R2 sub-pixel jitter over the screen-pixel box, averaged in linear RGB and presented gamma-corrected. Manual by default; an opt-in **auto AA** option (`renderOptions.aaAuto`) starts accumulation automatically once the view fully converges.
...
- Keep **analytic-AA eligibility conservative and palette-independent**: use the original `|z′| / (|z″| δ) > 5` Taylor-dominance certificate, reject every non-finite payload, and require a cheap whole-footprint lower bound on the retained reconstruction to remain beyond bailout.
...
- **Math** (`src/Mandelbrot.ts`): screen-aligned, finite-prefix-balanced R2 sequence over a box footprint.
