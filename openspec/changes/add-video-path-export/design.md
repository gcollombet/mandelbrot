...
### 10. Montage commun avec ExpMap

Le sélecteur distingue « Vidéo depuis la fractale » et « Vidéo depuis une ExpMap ». Les contrôles Vue de rotation et transitions sont partagés : degrés, tours fractionnaires, sens, courbes d’entrée/sortie et durées indépendantes, palier final fixe et raccourci minibrot. Le mode fractale propose deux sliders d’échelle +10 à -1000, des captures indépendantes (centre commun ou centres distincts, zoom de chaque borne), aperçu de chaque borne, inversion et magnitude entière. Les nouvelles valeurs ne suivent pas implicitement les déplacements de la caméra. Les préférences conservent les coordonnées exactes, les tours, les transitions, le nom du fichier et la priorité durée/vitesse. La sortie initiale est 4K/60 et Auto préfère HEVC puis H.264 selon la sonde réelle ; les codecs explicitement choisis sont respectés.

Le runner transmet une progression déjà remappée au navigateur via `start_export_transition`. Cette entrée export utilise une progression linéaire et l’angle déroulé exact, tandis que `start_transition` conserve la courbe douce et le chemin court de la navigation interactive. Pour les ratios d’échelles hors f64, le facteur intermédiaire est reconstruit en séparant mantisse et exposant décimal. Les coordonnées ne sont jamais converties en f64. La boucle commune aux modes plein cadre et keyframes tuilées compte le palier dans la durée encodée ; le temps caméra/animations est figé à l’arrivée pendant ce palier. Les timestamps d’encodage restent uniformes, indépendants du ralentissement et du coût des frames.
