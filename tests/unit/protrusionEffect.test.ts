...
describe('palette protrusion effect', () => {
  ...
  it('keeps one row-6 sample and one styled scale for analytic lighting cues', () => {
    ...
    expect(shader).toContain('let geometricProfile = sign(geometricCarrier) * pow(abs(geometricCarrier), protrusionSharpness);');
    expect(shader).toContain('let geometricProtrusionGain = max(1.0 + fx.protrusion * geometricProfile, 0.0);');
