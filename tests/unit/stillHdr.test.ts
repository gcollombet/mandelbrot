...
function fixture(fail = false) {
  ...
  const deps: StillExportDeps = {
    engine: {
      ...
      captureHdrFrame: vi.fn(async (w,h) => { if (fail) throw new Error('GPU capture failure'); return new Uint16Array(w*h*3).fill(++tile) }),
    },
...
describe('HDR still session', () => {
  it('assembles GPU-converted RGB16 tiles without a VideoFrame or 8-bit canvas round trip and restores the camera', async () => {
    const { deps, ctx } = fixture()
    const result = await renderStill(deps, request)
    expect(Array.from(result.hdrPixels!.filter((_,i) => i%3 === 0))).toEqual([1,1,2,2,1,1,2,2,3,3,4,4,3,3,4,4])
    ...
    expect(deps.engine.captureHdrFrame).toHaveBeenLastCalledWith(2,2,1,expect.objectContaining({format:'png',originX:2,originY:2,exposure:0}))
  ...
  it('assembles all tiles and reports clipping only once for the capture',async()=>{
    const {deps}=fixture(),onWarning=vi.fn()
    deps.engine.captureHdrFrame=vi.fn(async(w,h,_ss,options)=>{
      options?.onWarning?.('Clipped HDR');return new Uint16Array(w*h*3)
    })
    const result=await renderStill(deps,{...request,onWarning})
    expect(result.hdrPixels).toHaveLength(4*4*3)
    expect(deps.engine.captureHdrFrame).toHaveBeenCalledTimes(4)
    expect(onWarning).toHaveBeenCalledExactlyOnceWith('Clipped HDR')
  })
