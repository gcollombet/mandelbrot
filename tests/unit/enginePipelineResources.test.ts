...
describe('asynchronous iteration pipelines', () => {
    ...
    it.each(['mode', 'resize', 'options', 'destroy'])(
        'does not encode a stale frame when %s changes during compilation', async change => {
            ...
            if (change === 'mode') engine.floatExpActive = !engine.floatExpActive
...
describe('in-place kernel compilation', () => {
    ...
    it('specialises on ENABLE_DEEP only', () => {
        const engine = makeEngine()
        engine.inplacePipelineLayout = {}
        engine.inplaceModule = {}
        const {key, descriptor} = engine.inplacePipelineSpec(true)
        expect(key).toBe('d1')
        expect(descriptor.compute.constants).toEqual({ENABLE_DEEP: 1})
        expect(engine.inplacePipelineSpec(false).descriptor.compute.constants).toEqual({ENABLE_DEEP: 0})
    })

    it('keeps both kernels when the driver accepts them', async () => {
        const engine = makeEngine()
        engine.device = {createComputePipelineAsync: refusing(() => false)}
        const {deep, shallow} = await engine.compileInplacePipelines()
        expect(deep.constants).toEqual({ENABLE_DEEP: 1})
        expect(shallow.constants).toEqual({ENABLE_DEEP: 0})
    ...
    it('fails initialisation with the driver error when the shallow kernel is refused', async () => {
        const engine = makeEngine()
        engine.device = {createComputePipelineAsync: refusing(c => c.ENABLE_DEEP === 0)}
