(async ()=>{
    var qe = "" + new URL("mandelbrot_bg-C2OlfLfZ.wasm", import.meta.url).href, e_ = async (n = {}, e)=>{
        let i;
        if (e.startsWith("data:")) {
            const r = e.replace(/^data:.*?base64,/, "");
            let a;
            if (typeof Buffer == "function" && typeof Buffer.from == "function") a = Buffer.from(r, "base64");
            else if (typeof atob == "function") {
                const s = atob(r);
                a = new Uint8Array(s.length);
                for(let o = 0; o < s.length; o++)a[o] = s.charCodeAt(o);
            } else throw new Error("Cannot decode base64-encoded data URL");
            i = await WebAssembly.instantiate(a, n);
        } else {
            const r = await fetch(e), a = r.headers.get("Content-Type") || "";
            if ("instantiateStreaming" in WebAssembly && a.startsWith("application/wasm")) i = await WebAssembly.instantiateStreaming(r, n);
            else {
                const s = await r.arrayBuffer();
                i = await WebAssembly.instantiate(s, n);
            }
        }
        return i.instance.exports;
    };
    let t;
    function __(n) {
        t = n;
    }
    let se = null;
    function le() {
        return (se === null || se.byteLength === 0) && (se = new Uint8Array(t.memory.buffer)), se;
    }
    let ge = new TextDecoder("utf-8", {
        ignoreBOM: !0,
        fatal: !0
    });
    ge.decode();
    const t_ = 2146435072;
    let he = 0;
    function n_(n, e) {
        return he += e, he >= t_ && (ge = new TextDecoder("utf-8", {
            ignoreBOM: !0,
            fatal: !0
        }), ge.decode(), he = e), ge.decode(le().subarray(n, n + e));
    }
    function Ve(n, e) {
        return n = n >>> 0, n_(n, e);
    }
    let P = null;
    function i_() {
        return (P === null || P.buffer.detached === !0 || P.buffer.detached === void 0 && P.buffer !== t.memory.buffer) && (P = new DataView(t.memory.buffer)), P;
    }
    function W(n, e) {
        n = n >>> 0;
        const i = i_(), r = [];
        for(let a = n; a < n + 4 * e; a += 4)r.push(t.__wbindgen_export_0.get(i.getUint32(a, !0)));
        return t.__externref_drop_slice(n, e), r;
    }
    let be = null;
    function r_() {
        return (be === null || be.byteLength === 0) && (be = new Float64Array(t.memory.buffer)), be;
    }
    function Ee(n, e) {
        return n = n >>> 0, r_().subarray(n / 8, n / 8 + e);
    }
    let v = 0;
    const te = new TextEncoder;
    "encodeInto" in te || (te.encodeInto = function(n, e) {
        const i = te.encode(n);
        return e.set(i), {
            read: n.length,
            written: i.length
        };
    });
    function k(n, e, i) {
        if (i === void 0) {
            const c = te.encode(n), l = e(c.length, 1) >>> 0;
            return le().subarray(l, l + c.length).set(c), v = c.length, l;
        }
        let r = n.length, a = e(r, 1) >>> 0;
        const s = le();
        let o = 0;
        for(; o < r; o++){
            const c = n.charCodeAt(o);
            if (c > 127) break;
            s[a + o] = c;
        }
        if (o !== r) {
            o !== 0 && (n = n.slice(o)), a = i(a, r, r = o + n.length * 3, 1) >>> 0;
            const c = le().subarray(a + o, a + r), l = te.encodeInto(n, c);
            o += l.written, a = i(a, r, o, 1) >>> 0;
        }
        return v = o, a;
    }
    function T(n) {
        return n == null;
    }
    const Te = typeof FinalizationRegistry > "u" ? {
        register: ()=>{},
        unregister: ()=>{}
    } : new FinalizationRegistry((n)=>t.__wbg_blabufferinfo_free(n >>> 0, 1));
    class ne {
        static __wrap(e) {
            e = e >>> 0;
            const i = Object.create(ne.prototype);
            return i.__wbg_ptr = e, Te.register(i, i.__wbg_ptr, i), i;
        }
        __destroy_into_raw() {
            const e = this.__wbg_ptr;
            return this.__wbg_ptr = 0, Te.unregister(this), e;
        }
        free() {
            const e = this.__destroy_into_raw();
            t.__wbg_blabufferinfo_free(e, 0);
        }
        get ptr() {
            return t.__wbg_get_blabufferinfo_ptr(this.__wbg_ptr) >>> 0;
        }
        set ptr(e) {
            t.__wbg_set_blabufferinfo_ptr(this.__wbg_ptr, e);
        }
        get count() {
            return t.__wbg_get_blabufferinfo_count(this.__wbg_ptr) >>> 0;
        }
        set count(e) {
            t.__wbg_set_blabufferinfo_count(this.__wbg_ptr, e);
        }
        get levels_ptr() {
            return t.__wbg_get_blabufferinfo_levels_ptr(this.__wbg_ptr) >>> 0;
        }
        set levels_ptr(e) {
            t.__wbg_set_blabufferinfo_levels_ptr(this.__wbg_ptr, e);
        }
        get level_count() {
            return t.__wbg_get_blabufferinfo_level_count(this.__wbg_ptr) >>> 0;
        }
        set level_count(e) {
            t.__wbg_set_blabufferinfo_level_count(this.__wbg_ptr, e);
        }
    }
    Symbol.dispose && (ne.prototype[Symbol.dispose] = ne.prototype.free);
    typeof FinalizationRegistry > "u" || new FinalizationRegistry((n)=>t.__wbg_blalevel_free(n >>> 0, 1));
    typeof FinalizationRegistry > "u" || new FinalizationRegistry((n)=>t.__wbg_blastep_free(n >>> 0, 1));
    const ze = typeof FinalizationRegistry > "u" ? {
        register: ()=>{},
        unregister: ()=>{}
    } : new FinalizationRegistry((n)=>t.__wbg_incrementalunifiedbufferinfo_free(n >>> 0, 1));
    class ie {
        static __wrap(e) {
            e = e >>> 0;
            const i = Object.create(ie.prototype);
            return i.__wbg_ptr = e, ze.register(i, i.__wbg_ptr, i), i;
        }
        __destroy_into_raw() {
            const e = this.__wbg_ptr;
            return this.__wbg_ptr = 0, ze.unregister(this), e;
        }
        free() {
            const e = this.__destroy_into_raw();
            t.__wbg_incrementalunifiedbufferinfo_free(e, 0);
        }
        get ranges_ptr() {
            return t.__wbg_get_incrementalunifiedbufferinfo_ranges_ptr(this.__wbg_ptr) >>> 0;
        }
        set ranges_ptr(e) {
            t.__wbg_set_incrementalunifiedbufferinfo_ranges_ptr(this.__wbg_ptr, e);
        }
        get range_count() {
            return t.__wbg_get_incrementalunifiedbufferinfo_range_count(this.__wbg_ptr) >>> 0;
        }
        set range_count(e) {
            t.__wbg_set_incrementalunifiedbufferinfo_range_count(this.__wbg_ptr, e);
        }
        get coeffs_ptr() {
            return t.__wbg_get_incrementalunifiedbufferinfo_coeffs_ptr(this.__wbg_ptr) >>> 0;
        }
        set coeffs_ptr(e) {
            t.__wbg_set_incrementalunifiedbufferinfo_coeffs_ptr(this.__wbg_ptr, e);
        }
        get coeffs_count() {
            return t.__wbg_get_incrementalunifiedbufferinfo_coeffs_count(this.__wbg_ptr) >>> 0;
        }
        set coeffs_count(e) {
            t.__wbg_set_blastep_d_exp(this.__wbg_ptr, e);
        }
        get radii_ptr() {
            return t.__wbg_get_incrementalunifiedbufferinfo_radii_ptr(this.__wbg_ptr) >>> 0;
        }
        set radii_ptr(e) {
            t.__wbg_set_incrementalunifiedbufferinfo_radii_ptr(this.__wbg_ptr, e);
        }
        get radii_count() {
            return t.__wbg_get_incrementalunifiedbufferinfo_radii_count(this.__wbg_ptr) >>> 0;
        }
        set radii_count(e) {
            t.__wbg_set_incrementalunifiedbufferinfo_radii_count(this.__wbg_ptr, e);
        }
        get certificates_ptr() {
            return t.__wbg_get_incrementalunifiedbufferinfo_certificates_ptr(this.__wbg_ptr) >>> 0;
        }
        set certificates_ptr(e) {
            t.__wbg_set_incrementalunifiedbufferinfo_certificates_ptr(this.__wbg_ptr, e);
        }
        get certificates_count() {
            return t.__wbg_get_incrementalunifiedbufferinfo_certificates_count(this.__wbg_ptr) >>> 0;
        }
        set certificates_count(e) {
            t.__wbg_set_incrementalunifiedbufferinfo_certificates_count(this.__wbg_ptr, e);
        }
        get certificate_version() {
            return t.__wbg_get_incrementalunifiedbufferinfo_certificate_version(this.__wbg_ptr) >>> 0;
        }
        set certificate_version(e) {
            t.__wbg_set_incrementalunifiedbufferinfo_certificate_version(this.__wbg_ptr, e);
        }
        get certificate_words_per_block() {
            return t.__wbg_get_incrementalunifiedbufferinfo_certificate_words_per_block(this.__wbg_ptr) >>> 0;
        }
        set certificate_words_per_block(e) {
            t.__wbg_set_incrementalunifiedbufferinfo_certificate_words_per_block(this.__wbg_ptr, e);
        }
        get reference_log2_dc() {
            return t.__wbg_get_incrementalunifiedbufferinfo_reference_log2_dc(this.__wbg_ptr);
        }
        set reference_log2_dc(e) {
            t.__wbg_set_incrementalunifiedbufferinfo_reference_log2_dc(this.__wbg_ptr, e);
        }
        get covered_orbit_len() {
            return t.__wbg_get_incrementalunifiedbufferinfo_covered_orbit_len(this.__wbg_ptr) >>> 0;
        }
        set covered_orbit_len(e) {
            t.__wbg_set_incrementalunifiedbufferinfo_covered_orbit_len(this.__wbg_ptr, e);
        }
        get published_orbit_len() {
            return t.__wbg_get_incrementalunifiedbufferinfo_published_orbit_len(this.__wbg_ptr) >>> 0;
        }
        set published_orbit_len(e) {
            t.__wbg_set_incrementalunifiedbufferinfo_published_orbit_len(this.__wbg_ptr, e);
        }
        get reset() {
            return t.__wbg_get_incrementalunifiedbufferinfo_reset(this.__wbg_ptr) >>> 0;
        }
        set reset(e) {
            t.__wbg_set_incrementalunifiedbufferinfo_reset(this.__wbg_ptr, e);
        }
        get has_more() {
            return t.__wbg_get_incrementalunifiedbufferinfo_has_more(this.__wbg_ptr) >>> 0;
        }
        set has_more(e) {
            t.__wbg_set_incrementalunifiedbufferinfo_has_more(this.__wbg_ptr, e);
        }
        get cumulative_merges() {
            return t.__wbg_get_incrementalunifiedbufferinfo_cumulative_merges(this.__wbg_ptr) >>> 0;
        }
        set cumulative_merges(e) {
            t.__wbg_set_incrementalunifiedbufferinfo_cumulative_merges(this.__wbg_ptr, e);
        }
        get cumulative_coefficients() {
            return t.__wbg_get_incrementalunifiedbufferinfo_cumulative_coefficients(this.__wbg_ptr) >>> 0;
        }
        set cumulative_coefficients(e) {
            t.__wbg_set_incrementalunifiedbufferinfo_cumulative_coefficients(this.__wbg_ptr, e);
        }
        get cumulative_envelopes() {
            return t.__wbg_get_incrementalunifiedbufferinfo_cumulative_envelopes(this.__wbg_ptr) >>> 0;
        }
        set cumulative_envelopes(e) {
            t.__wbg_set_incrementalunifiedbufferinfo_cumulative_envelopes(this.__wbg_ptr, e);
        }
        get peak_retained_bytes() {
            return t.__wbg_get_incrementalunifiedbufferinfo_peak_retained_bytes(this.__wbg_ptr) >>> 0;
        }
        set peak_retained_bytes(e) {
            t.__wbg_set_incrementalunifiedbufferinfo_peak_retained_bytes(this.__wbg_ptr, e);
        }
        get cumulative_merge_coefficients_ms() {
            return t.__wbg_get_incrementalunifiedbufferinfo_cumulative_merge_coefficients_ms(this.__wbg_ptr);
        }
        set cumulative_merge_coefficients_ms(e) {
            t.__wbg_set_incrementalunifiedbufferinfo_cumulative_merge_coefficients_ms(this.__wbg_ptr, e);
        }
        get cumulative_envelope_ms() {
            return t.__wbg_get_incrementalunifiedbufferinfo_cumulative_envelope_ms(this.__wbg_ptr);
        }
        set cumulative_envelope_ms(e) {
            t.__wbg_set_incrementalunifiedbufferinfo_cumulative_envelope_ms(this.__wbg_ptr, e);
        }
    }
    Symbol.dispose && (ie.prototype[Symbol.dispose] = ie.prototype.free);
    typeof FinalizationRegistry > "u" || new FinalizationRegistry((n)=>t.__wbg_incrementalunifiedrangeinfo_free(n >>> 0, 1));
    const Ne = typeof FinalizationRegistry > "u" ? {
        register: ()=>{},
        unregister: ()=>{}
    } : new FinalizationRegistry((n)=>t.__wbg_jetbufferinfo_free(n >>> 0, 1));
    class re {
        static __wrap(e) {
            e = e >>> 0;
            const i = Object.create(re.prototype);
            return i.__wbg_ptr = e, Ne.register(i, i.__wbg_ptr, i), i;
        }
        __destroy_into_raw() {
            const e = this.__wbg_ptr;
            return this.__wbg_ptr = 0, Ne.unregister(this), e;
        }
        free() {
            const e = this.__destroy_into_raw();
            t.__wbg_jetbufferinfo_free(e, 0);
        }
        get coeffs_ptr() {
            return t.__wbg_get_blabufferinfo_ptr(this.__wbg_ptr) >>> 0;
        }
        set coeffs_ptr(e) {
            t.__wbg_set_blabufferinfo_ptr(this.__wbg_ptr, e);
        }
        get coeffs_count() {
            return t.__wbg_get_blabufferinfo_count(this.__wbg_ptr) >>> 0;
        }
        set coeffs_count(e) {
            t.__wbg_set_blabufferinfo_count(this.__wbg_ptr, e);
        }
        get radii_ptr() {
            return t.__wbg_get_blabufferinfo_levels_ptr(this.__wbg_ptr) >>> 0;
        }
        set radii_ptr(e) {
            t.__wbg_set_blabufferinfo_levels_ptr(this.__wbg_ptr, e);
        }
        get radii_count() {
            return t.__wbg_get_blabufferinfo_level_count(this.__wbg_ptr) >>> 0;
        }
        set radii_count(e) {
            t.__wbg_set_blabufferinfo_level_count(this.__wbg_ptr, e);
        }
        get levels_ptr() {
            return t.__wbg_get_incrementalunifiedrangeinfo_payload_offset(this.__wbg_ptr) >>> 0;
        }
        set levels_ptr(e) {
            t.__wbg_set_blastep_ab_exp(this.__wbg_ptr, e);
        }
        get level_count() {
            return t.__wbg_get_incrementalunifiedrangeinfo_committed_count(this.__wbg_ptr) >>> 0;
        }
        set level_count(e) {
            t.__wbg_set_incrementalunifiedrangeinfo_committed_count(this.__wbg_ptr, e);
        }
    }
    Symbol.dispose && (re.prototype[Symbol.dispose] = re.prototype.free);
    const Be = typeof FinalizationRegistry > "u" ? {
        register: ()=>{},
        unregister: ()=>{}
    } : new FinalizationRegistry((n)=>t.__wbg_mandelbrotnavigator_free(n >>> 0, 1));
    class xe {
        __destroy_into_raw() {
            const e = this.__wbg_ptr;
            return this.__wbg_ptr = 0, Be.unregister(this), e;
        }
        free() {
            const e = this.__destroy_into_raw();
            t.__wbg_mandelbrotnavigator_free(e, 0);
        }
        get_params() {
            const e = t.mandelbrotnavigator_get_params(this.__wbg_ptr);
            var i = W(e[0], e[1]).slice();
            return t.__wbindgen_free(e[0], e[1] * 4, 4), i;
        }
        use_unified() {
            t.mandelbrotnavigator_use_unified(this.__wbg_ptr);
        }
        find_minibrot(e, i) {
            const r = t.mandelbrotnavigator_find_minibrot(this.__wbg_ptr, e, i);
            var a = W(r[0], r[1]).slice();
            return t.__wbindgen_free(r[0], r[1] * 4, 4), a;
        }
        rotate_direct(e) {
            t.mandelbrotnavigator_rotate_direct(this.__wbg_ptr, e);
        }
        view_floatexp() {
            const e = t.mandelbrotnavigator_view_floatexp(this.__wbg_ptr);
            var i = Ee(e[0], e[1]).slice();
            return t.__wbindgen_free(e[0], e[1] * 8, 8), i;
        }
        benchmark_pade(e) {
            const i = t.mandelbrotnavigator_benchmark_pade(this.__wbg_ptr, e);
            return ae.__wrap(i);
        }
        get_bla_epsilon() {
            return t.mandelbrotnavigator_get_bla_epsilon(this.__wbg_ptr);
        }
        set_bla_epsilon(e) {
            t.mandelbrotnavigator_set_bla_epsilon(this.__wbg_ptr, e);
        }
        unified_is_cold(e) {
            return t.mandelbrotnavigator_unified_is_cold(this.__wbg_ptr, e) !== 0;
        }
        get_max_bla_skip() {
            return t.mandelbrotnavigator_get_max_bla_skip(this.__wbg_ptr) >>> 0;
        }
        is_in_transition() {
            return t.mandelbrotnavigator_is_in_transition(this.__wbg_ptr) !== 0;
        }
        pixel_to_complex(e, i, r, a) {
            const s = t.mandelbrotnavigator_pixel_to_complex(this.__wbg_ptr, e, i, r, a);
            var o = W(s[0], s[1]).slice();
            return t.__wbindgen_free(s[0], s[1] * 4, 4), o;
        }
        reference_origin(e, i) {
            const r = k(e, t.__wbindgen_malloc, t.__wbindgen_realloc), a = v, s = k(i, t.__wbindgen_malloc, t.__wbindgen_realloc), o = v;
            t.mandelbrotnavigator_reference_origin(this.__wbg_ptr, r, a, s, o);
        }
        set_max_bla_skip(e) {
            t.mandelbrotnavigator_set_max_bla_skip(this.__wbg_ptr, e);
        }
        start_transition(e, i, r, a, s) {
            const o = k(e, t.__wbindgen_malloc, t.__wbindgen_realloc), c = v, l = k(i, t.__wbindgen_malloc, t.__wbindgen_realloc), g = v, w = k(r, t.__wbindgen_malloc, t.__wbindgen_realloc), u = v;
            t.mandelbrotnavigator_start_transition(this.__wbg_ptr, o, c, l, g, w, u, a, s);
        }
        translate_direct(e, i, r, a) {
            t.mandelbrotnavigator_translate_direct(this.__wbg_ptr, e, i, !T(r), T(r) ? 0 : r, !T(a), T(a) ? 0 : a);
        }
        use_mobius_cplus() {
            t.mandelbrotnavigator_use_mobius_cplus(this.__wbg_ptr);
        }
        use_perturbation() {
            t.mandelbrotnavigator_use_perturbation(this.__wbg_ptr);
        }
        cancel_transition() {
            t.mandelbrotnavigator_cancel_transition(this.__wbg_ptr);
        }
        get_gate_emission() {
            return t.mandelbrotnavigator_get_gate_emission(this.__wbg_ptr) !== 0;
        }
        set_gate_emission(e) {
            t.mandelbrotnavigator_set_gate_emission(this.__wbg_ptr, e);
        }
        current_log2_c_max() {
            return t.mandelbrotnavigator_current_log2_c_max(this.__wbg_ptr);
        }
        unified_last_sa_n0() {
            return t.mandelbrotnavigator_unified_last_sa_n0(this.__wbg_ptr) >>> 0;
        }
        coordinate_to_pixel(e, i, r, a) {
            const s = k(e, t.__wbindgen_malloc, t.__wbindgen_realloc), o = v, c = k(i, t.__wbindgen_malloc, t.__wbindgen_realloc), l = v, g = t.mandelbrotnavigator_coordinate_to_pixel(this.__wbg_ptr, s, o, c, l, r, a);
            var w = Ee(g[0], g[1]).slice();
            return t.__wbindgen_free(g[0], g[1] * 8, 8), w;
        }
        set_viewport_aspect(e) {
            t.mandelbrotnavigator_set_viewport_aspect(this.__wbg_ptr, e);
        }
        unified_last_stages() {
            return t.mandelbrotnavigator_unified_last_stages(this.__wbg_ptr) >>> 0;
        }
        find_minibrot_framed(e, i, r) {
            const a = t.mandelbrotnavigator_find_minibrot_framed(this.__wbg_ptr, e, i, r);
            var s = W(a[0], a[1]).slice();
            return t.__wbindgen_free(a[0], a[1] * 4, 4), s;
        }
        get_reference_params() {
            const e = t.mandelbrotnavigator_get_reference_params(this.__wbg_ptr);
            var i = W(e[0], e[1]).slice();
            return t.__wbindgen_free(e[0], e[1] * 4, 4), i;
        }
        set_precision_budget(e) {
            const i = k(e, t.__wbindgen_malloc, t.__wbindgen_realloc), r = v;
            t.mandelbrotnavigator_set_precision_budget(this.__wbg_ptr, i, r);
        }
        compute_jet_reference(e) {
            const i = t.mandelbrotnavigator_compute_jet_reference(this.__wbg_ptr, e);
            return re.__wrap(i);
        }
        compute_unified_header(e) {
            const i = t.mandelbrotnavigator_compute_unified_header(this.__wbg_ptr, e);
            return O.__wrap(i);
        }
        get_approximation_mode() {
            return t.mandelbrotnavigator_get_approximation_mode(this.__wbg_ptr);
        }
        unified_last_band_log2() {
            return t.mandelbrotnavigator_unified_last_band_log2(this.__wbg_ptr);
        }
        begin_unified_reference(e) {
            t.mandelbrotnavigator_begin_unified_reference(this.__wbg_ptr, e);
        }
        get_reference_orbit_len() {
            return t.mandelbrotnavigator_get_reference_orbit_len(this.__wbg_ptr) >>> 0;
        }
        unified_last_gate_count() {
            return t.mandelbrotnavigator_unified_last_gate_count(this.__wbg_ptr) >>> 0;
        }
        unified_last_periodic_p() {
            return t.mandelbrotnavigator_unified_last_periodic_p(this.__wbg_ptr) >>> 0;
        }
        compute_mobius_reference(e) {
            const i = t.mandelbrotnavigator_compute_mobius_reference(this.__wbg_ptr, e);
            return oe.__wrap(i);
        }
        finish_unified_reference(e) {
            const i = t.mandelbrotnavigator_finish_unified_reference(this.__wbg_ptr, e);
            return O.__wrap(i);
        }
        unified_last_band_spread() {
            return t.mandelbrotnavigator_unified_last_band_spread(this.__wbg_ptr);
        }
        compute_bla_reference_ptr(e) {
            const i = t.mandelbrotnavigator_compute_bla_reference_ptr(this.__wbg_ptr, e);
            return ne.__wrap(i);
        }
        compute_unified_reference(e) {
            const i = t.mandelbrotnavigator_compute_unified_reference(this.__wbg_ptr, e);
            return O.__wrap(i);
        }
        get_dynamic_block_validity() {
            return t.mandelbrotnavigator_get_dynamic_block_validity(this.__wbg_ptr) !== 0;
        }
        set_dynamic_block_validity(e) {
            t.mandelbrotnavigator_set_dynamic_block_validity(this.__wbg_ptr, e);
        }
        compute_reference_orbit_ptr(e) {
            const i = t.mandelbrotnavigator_compute_reference_orbit_ptr(this.__wbg_ptr, e);
            return K.__wrap(i);
        }
        get_reference_orbit_capacity() {
            return t.mandelbrotnavigator_get_reference_orbit_capacity(this.__wbg_ptr) >>> 0;
        }
        unified_last_periodic_status() {
            return t.mandelbrotnavigator_unified_last_periodic_status(this.__wbg_ptr) >>> 0;
        }
        compute_reference_orbit_chunk(e, i) {
            const r = t.mandelbrotnavigator_compute_reference_orbit_chunk(this.__wbg_ptr, e, i);
            return K.__wrap(r);
        }
        get_incremental_reference_table() {
            return t.mandelbrotnavigator_get_incremental_reference_table(this.__wbg_ptr) !== 0;
        }
        set_incremental_reference_table(e) {
            t.mandelbrotnavigator_set_incremental_reference_table(this.__wbg_ptr, e);
        }
        unified_last_periodic_detected_p() {
            return t.mandelbrotnavigator_unified_last_periodic_detected_p(this.__wbg_ptr) >>> 0;
        }
        continue_unified_reference_bounds(e) {
            t.mandelbrotnavigator_continue_unified_reference_bounds(this.__wbg_ptr, e);
        }
        advance_incremental_unified_reference(e, i, r) {
            const a = t.mandelbrotnavigator_advance_incremental_unified_reference(this.__wbg_ptr, e, i, r);
            return ie.__wrap(a);
        }
        constructor(e, i, r, a){
            const s = k(e, t.__wbindgen_malloc, t.__wbindgen_realloc), o = v, c = k(i, t.__wbindgen_malloc, t.__wbindgen_realloc), l = v, g = k(r, t.__wbindgen_malloc, t.__wbindgen_realloc), w = v, u = t.mandelbrotnavigator_new(s, o, c, l, g, w, a);
            return this.__wbg_ptr = u >>> 0, Be.register(this, this.__wbg_ptr, this), this;
        }
        step(e, i) {
            const r = t.mandelbrotnavigator_step(this.__wbg_ptr, !T(e), T(e) ? 0 : e, !T(i), T(i) ? 0 : i);
            var a = W(r[0], r[1]).slice();
            return t.__wbindgen_free(r[0], r[1] * 4, 4), a;
        }
        zoom(e) {
            t.mandelbrotnavigator_zoom(this.__wbg_ptr, e);
        }
        angle(e) {
            t.mandelbrotnavigator_angle(this.__wbg_ptr, e);
        }
        scale(e) {
            const i = k(e, t.__wbindgen_malloc, t.__wbindgen_realloc), r = v;
            t.mandelbrotnavigator_scale(this.__wbg_ptr, i, r);
        }
        origin(e, i) {
            const r = k(e, t.__wbindgen_malloc, t.__wbindgen_realloc), a = v, s = k(i, t.__wbindgen_malloc, t.__wbindgen_realloc), o = v;
            t.mandelbrotnavigator_origin(this.__wbg_ptr, r, a, s, o);
        }
        rotate(e) {
            t.mandelbrotnavigator_rotate(this.__wbg_ptr, e);
        }
        use_bla() {
            t.mandelbrotnavigator_use_bla(this.__wbg_ptr);
        }
        use_jet() {
            t.mandelbrotnavigator_use_jet(this.__wbg_ptr);
        }
        use_pade() {
            t.mandelbrotnavigator_use_pade(this.__wbg_ptr);
        }
        translate(e, i) {
            t.mandelbrotnavigator_translate(this.__wbg_ptr, e, i);
        }
    }
    Symbol.dispose && (xe.prototype[Symbol.dispose] = xe.prototype.free);
    typeof FinalizationRegistry > "u" || new FinalizationRegistry((n)=>t.__wbg_mandelbrotstep_free(n >>> 0, 1));
    const Oe = typeof FinalizationRegistry > "u" ? {
        register: ()=>{},
        unregister: ()=>{}
    } : new FinalizationRegistry((n)=>t.__wbg_mobiusbufferinfo_free(n >>> 0, 1));
    class oe {
        static __wrap(e) {
            e = e >>> 0;
            const i = Object.create(oe.prototype);
            return i.__wbg_ptr = e, Oe.register(i, i.__wbg_ptr, i), i;
        }
        __destroy_into_raw() {
            const e = this.__wbg_ptr;
            return this.__wbg_ptr = 0, Oe.unregister(this), e;
        }
        free() {
            const e = this.__destroy_into_raw();
            t.__wbg_mobiusbufferinfo_free(e, 0);
        }
        get coeffs_ptr() {
            return t.__wbg_get_blabufferinfo_ptr(this.__wbg_ptr) >>> 0;
        }
        set coeffs_ptr(e) {
            t.__wbg_set_blabufferinfo_ptr(this.__wbg_ptr, e);
        }
        get coeffs_count() {
            return t.__wbg_get_blabufferinfo_count(this.__wbg_ptr) >>> 0;
        }
        set coeffs_count(e) {
            t.__wbg_set_blabufferinfo_count(this.__wbg_ptr, e);
        }
        get radii_ptr() {
            return t.__wbg_get_blabufferinfo_levels_ptr(this.__wbg_ptr) >>> 0;
        }
        set radii_ptr(e) {
            t.__wbg_set_blabufferinfo_levels_ptr(this.__wbg_ptr, e);
        }
        get radii_count() {
            return t.__wbg_get_blabufferinfo_level_count(this.__wbg_ptr) >>> 0;
        }
        set radii_count(e) {
            t.__wbg_set_blabufferinfo_level_count(this.__wbg_ptr, e);
        }
        get levels_ptr() {
            return t.__wbg_get_incrementalunifiedrangeinfo_payload_offset(this.__wbg_ptr) >>> 0;
        }
        set levels_ptr(e) {
            t.__wbg_set_blastep_ab_exp(this.__wbg_ptr, e);
        }
        get level_count() {
            return t.__wbg_get_incrementalunifiedrangeinfo_committed_count(this.__wbg_ptr) >>> 0;
        }
        set level_count(e) {
            t.__wbg_set_incrementalunifiedrangeinfo_committed_count(this.__wbg_ptr, e);
        }
    }
    Symbol.dispose && (oe.prototype[Symbol.dispose] = oe.prototype.free);
    const De = typeof FinalizationRegistry > "u" ? {
        register: ()=>{},
        unregister: ()=>{}
    } : new FinalizationRegistry((n)=>t.__wbg_orbitbufferinfo_free(n >>> 0, 1));
    class K {
        static __wrap(e) {
            e = e >>> 0;
            const i = Object.create(K.prototype);
            return i.__wbg_ptr = e, De.register(i, i.__wbg_ptr, i), i;
        }
        __destroy_into_raw() {
            const e = this.__wbg_ptr;
            return this.__wbg_ptr = 0, De.unregister(this), e;
        }
        free() {
            const e = this.__destroy_into_raw();
            t.__wbg_orbitbufferinfo_free(e, 0);
        }
        get ptr() {
            return t.__wbg_get_blabufferinfo_ptr(this.__wbg_ptr) >>> 0;
        }
        set ptr(e) {
            t.__wbg_set_blabufferinfo_ptr(this.__wbg_ptr, e);
        }
        get offset() {
            return t.__wbg_get_blabufferinfo_count(this.__wbg_ptr) >>> 0;
        }
        set offset(e) {
            t.__wbg_set_blabufferinfo_count(this.__wbg_ptr, e);
        }
        get count() {
            return t.__wbg_get_blabufferinfo_levels_ptr(this.__wbg_ptr) >>> 0;
        }
        set count(e) {
            t.__wbg_set_blabufferinfo_levels_ptr(this.__wbg_ptr, e);
        }
    }
    Symbol.dispose && (K.prototype[Symbol.dispose] = K.prototype.free);
    const Le = typeof FinalizationRegistry > "u" ? {
        register: ()=>{},
        unregister: ()=>{}
    } : new FinalizationRegistry((n)=>t.__wbg_padebenchmark_free(n >>> 0, 1));
    class ae {
        static __wrap(e) {
            e = e >>> 0;
            const i = Object.create(ae.prototype);
            return i.__wbg_ptr = e, Le.register(i, i.__wbg_ptr, i), i;
        }
        __destroy_into_raw() {
            const e = this.__wbg_ptr;
            return this.__wbg_ptr = 0, Le.unregister(this), e;
        }
        free() {
            const e = this.__destroy_into_raw();
            t.__wbg_padebenchmark_free(e, 0);
        }
        get pixels() {
            return t.__wbg_get_incrementalunifiedbufferinfo_ranges_ptr(this.__wbg_ptr) >>> 0;
        }
        set pixels(e) {
            t.__wbg_set_incrementalunifiedbufferinfo_ranges_ptr(this.__wbg_ptr, e);
        }
        get max_iter() {
            return t.__wbg_get_incrementalunifiedbufferinfo_range_count(this.__wbg_ptr) >>> 0;
        }
        set max_iter(e) {
            t.__wbg_set_incrementalunifiedbufferinfo_range_count(this.__wbg_ptr, e);
        }
        get steps_exact() {
            return t.__wbg_get_incrementalunifiedbufferinfo_reference_log2_dc(this.__wbg_ptr);
        }
        set steps_exact(e) {
            t.__wbg_set_incrementalunifiedbufferinfo_reference_log2_dc(this.__wbg_ptr, e);
        }
        get steps_affine() {
            return t.__wbg_get_incrementalunifiedbufferinfo_cumulative_merge_coefficients_ms(this.__wbg_ptr);
        }
        set steps_affine(e) {
            t.__wbg_set_incrementalunifiedbufferinfo_cumulative_merge_coefficients_ms(this.__wbg_ptr, e);
        }
        get steps_pade() {
            return t.__wbg_get_incrementalunifiedbufferinfo_cumulative_envelope_ms(this.__wbg_ptr);
        }
        set steps_pade(e) {
            t.__wbg_set_incrementalunifiedbufferinfo_cumulative_envelope_ms(this.__wbg_ptr, e);
        }
        get pade_mismatches() {
            return t.__wbg_get_incrementalunifiedbufferinfo_coeffs_ptr(this.__wbg_ptr) >>> 0;
        }
        set pade_mismatches(e) {
            t.__wbg_set_incrementalunifiedbufferinfo_coeffs_ptr(this.__wbg_ptr, e);
        }
        get max_iter_delta() {
            return t.__wbg_get_incrementalunifiedbufferinfo_coeffs_count(this.__wbg_ptr) >>> 0;
        }
        set max_iter_delta(e) {
            t.__wbg_set_blastep_d_exp(this.__wbg_ptr, e);
        }
    }
    Symbol.dispose && (ae.prototype[Symbol.dispose] = ae.prototype.free);
    const Ue = typeof FinalizationRegistry > "u" ? {
        register: ()=>{},
        unregister: ()=>{}
    } : new FinalizationRegistry((n)=>t.__wbg_unifiedbufferinfo_free(n >>> 0, 1));
    class O {
        static __wrap(e) {
            e = e >>> 0;
            const i = Object.create(O.prototype);
            return i.__wbg_ptr = e, Ue.register(i, i.__wbg_ptr, i), i;
        }
        __destroy_into_raw() {
            const e = this.__wbg_ptr;
            return this.__wbg_ptr = 0, Ue.unregister(this), e;
        }
        free() {
            const e = this.__destroy_into_raw();
            t.__wbg_unifiedbufferinfo_free(e, 0);
        }
        get coeffs_ptr() {
            return t.__wbg_get_incrementalunifiedbufferinfo_coeffs_ptr(this.__wbg_ptr) >>> 0;
        }
        set coeffs_ptr(e) {
            t.__wbg_set_incrementalunifiedbufferinfo_coeffs_ptr(this.__wbg_ptr, e);
        }
        get coeffs_count() {
            return t.__wbg_get_incrementalunifiedbufferinfo_coeffs_count(this.__wbg_ptr) >>> 0;
        }
        set coeffs_count(e) {
            t.__wbg_set_blastep_d_exp(this.__wbg_ptr, e);
        }
        get radii_ptr() {
            return t.__wbg_get_incrementalunifiedbufferinfo_radii_ptr(this.__wbg_ptr) >>> 0;
        }
        set radii_ptr(e) {
            t.__wbg_set_incrementalunifiedbufferinfo_radii_ptr(this.__wbg_ptr, e);
        }
        get radii_count() {
            return t.__wbg_get_incrementalunifiedbufferinfo_radii_count(this.__wbg_ptr) >>> 0;
        }
        set radii_count(e) {
            t.__wbg_set_incrementalunifiedbufferinfo_radii_count(this.__wbg_ptr, e);
        }
        get levels_ptr() {
            return t.__wbg_get_incrementalunifiedbufferinfo_certificates_ptr(this.__wbg_ptr) >>> 0;
        }
        set levels_ptr(e) {
            t.__wbg_set_incrementalunifiedbufferinfo_certificates_ptr(this.__wbg_ptr, e);
        }
        get level_count() {
            return t.__wbg_get_incrementalunifiedbufferinfo_certificates_count(this.__wbg_ptr) >>> 0;
        }
        set level_count(e) {
            t.__wbg_set_incrementalunifiedbufferinfo_certificates_count(this.__wbg_ptr, e);
        }
        get optional_headers_ptr() {
            return t.__wbg_get_incrementalunifiedbufferinfo_certificate_version(this.__wbg_ptr) >>> 0;
        }
        set optional_headers_ptr(e) {
            t.__wbg_set_incrementalunifiedbufferinfo_certificate_version(this.__wbg_ptr, e);
        }
        get optional_headers_count() {
            return t.__wbg_get_incrementalunifiedbufferinfo_certificate_words_per_block(this.__wbg_ptr) >>> 0;
        }
        set optional_headers_count(e) {
            t.__wbg_set_incrementalunifiedbufferinfo_certificate_words_per_block(this.__wbg_ptr, e);
        }
        get optional_headers_version() {
            return t.__wbg_get_incrementalunifiedbufferinfo_covered_orbit_len(this.__wbg_ptr) >>> 0;
        }
        set optional_headers_version(e) {
            t.__wbg_set_incrementalunifiedbufferinfo_covered_orbit_len(this.__wbg_ptr, e);
        }
        get optional_sa_log2_dc() {
            return t.__wbg_get_incrementalunifiedbufferinfo_reference_log2_dc(this.__wbg_ptr);
        }
        set optional_sa_log2_dc(e) {
            t.__wbg_set_incrementalunifiedbufferinfo_reference_log2_dc(this.__wbg_ptr, e);
        }
        get optional_periodic_log2_dc() {
            return t.__wbg_get_incrementalunifiedbufferinfo_cumulative_merge_coefficients_ms(this.__wbg_ptr);
        }
        set optional_periodic_log2_dc(e) {
            t.__wbg_set_incrementalunifiedbufferinfo_cumulative_merge_coefficients_ms(this.__wbg_ptr, e);
        }
        get optional_gate_log2_dc() {
            return t.__wbg_get_incrementalunifiedbufferinfo_cumulative_envelope_ms(this.__wbg_ptr);
        }
        set optional_gate_log2_dc(e) {
            t.__wbg_set_incrementalunifiedbufferinfo_cumulative_envelope_ms(this.__wbg_ptr, e);
        }
        get validity_ptr() {
            return t.__wbg_get_incrementalunifiedbufferinfo_published_orbit_len(this.__wbg_ptr) >>> 0;
        }
        set validity_ptr(e) {
            t.__wbg_set_incrementalunifiedbufferinfo_published_orbit_len(this.__wbg_ptr, e);
        }
        get validity_count() {
            return t.__wbg_get_incrementalunifiedbufferinfo_reset(this.__wbg_ptr) >>> 0;
        }
        set validity_count(e) {
            t.__wbg_set_incrementalunifiedbufferinfo_reset(this.__wbg_ptr, e);
        }
        get validity_diagnostics_ptr() {
            return t.__wbg_get_incrementalunifiedbufferinfo_has_more(this.__wbg_ptr) >>> 0;
        }
        set validity_diagnostics_ptr(e) {
            t.__wbg_set_incrementalunifiedbufferinfo_has_more(this.__wbg_ptr, e);
        }
        get validity_diagnostics_count() {
            return t.__wbg_get_incrementalunifiedbufferinfo_cumulative_merges(this.__wbg_ptr) >>> 0;
        }
        set validity_diagnostics_count(e) {
            t.__wbg_set_incrementalunifiedbufferinfo_cumulative_merges(this.__wbg_ptr, e);
        }
        get validity_diagnostics_words_per_block() {
            return t.__wbg_get_incrementalunifiedbufferinfo_cumulative_coefficients(this.__wbg_ptr) >>> 0;
        }
        set validity_diagnostics_words_per_block(e) {
            t.__wbg_set_incrementalunifiedbufferinfo_cumulative_coefficients(this.__wbg_ptr, e);
        }
        get validity_levels_ptr() {
            return t.__wbg_get_incrementalunifiedbufferinfo_cumulative_envelopes(this.__wbg_ptr) >>> 0;
        }
        set validity_levels_ptr(e) {
            t.__wbg_set_incrementalunifiedbufferinfo_cumulative_envelopes(this.__wbg_ptr, e);
        }
        get validity_level_count() {
            return t.__wbg_get_incrementalunifiedbufferinfo_peak_retained_bytes(this.__wbg_ptr) >>> 0;
        }
        set validity_level_count(e) {
            t.__wbg_set_incrementalunifiedbufferinfo_peak_retained_bytes(this.__wbg_ptr, e);
        }
        get validity_version() {
            return t.__wbg_get_unifiedbufferinfo_validity_version(this.__wbg_ptr) >>> 0;
        }
        set validity_version(e) {
            t.__wbg_set_unifiedbufferinfo_validity_version(this.__wbg_ptr, e);
        }
        get validity_words_per_block() {
            return t.__wbg_get_unifiedbufferinfo_validity_words_per_block(this.__wbg_ptr) >>> 0;
        }
        set validity_words_per_block(e) {
            t.__wbg_set_unifiedbufferinfo_validity_words_per_block(this.__wbg_ptr, e);
        }
        get validity_reference_log2_dc() {
            return t.__wbg_get_unifiedbufferinfo_validity_reference_log2_dc(this.__wbg_ptr);
        }
        set validity_reference_log2_dc(e) {
            t.__wbg_set_unifiedbufferinfo_validity_reference_log2_dc(this.__wbg_ptr, e);
        }
    }
    Symbol.dispose && (O.prototype[Symbol.dispose] = O.prototype.free);
    function o_(n) {
        return Math.exp(n);
    }
    function a_(n) {
        return Math.log(n);
    }
    function f_() {
        return Date.now();
    }
    function s_(n, e) {
        throw new Error(Ve(n, e));
    }
    function b_(n, e) {
        return Ve(n, e);
    }
    function c_() {
        const n = t.__wbindgen_export_0, e = n.grow(4);
        n.set(0, void 0), n.set(e + 0, void 0), n.set(e + 1, null), n.set(e + 2, !0), n.set(e + 3, !1);
    }
    URL = globalThis.URL;
    const _ = await e_({
        "./mandelbrot_bg.js": {
            __wbg_now_1e80617bcee43265: f_,
            __wbg_exp_9293ded1248e1bd3: o_,
            __wbg_log_5f75e13a39ba07fe: a_,
            __wbg_wbindgenthrow_451ec1a8469d7eb6: s_,
            __wbindgen_init_externref_table: c_,
            __wbindgen_cast_2241b6af4c4b2941: b_
        }
    }, qe), y = _.memory, l_ = _.__wbg_blabufferinfo_free, g_ = _.__wbg_blalevel_free, u_ = _.__wbg_blastep_free, d_ = _.__wbg_get_blabufferinfo_count, p_ = _.__wbg_get_blabufferinfo_level_count, w_ = _.__wbg_get_blabufferinfo_levels_ptr, m_ = _.__wbg_get_blabufferinfo_ptr, v_ = _.__wbg_get_blastep_ab_exp, h_ = _.__wbg_get_blastep_alpha_exp, y_ = _.__wbg_get_blastep_ax, k_ = _.__wbg_get_blastep_ay, x_ = _.__wbg_get_blastep_bx, R_ = _.__wbg_get_blastep_by, F_ = _.__wbg_get_blastep_d_exp, S_ = _.__wbg_get_blastep_dx, A_ = _.__wbg_get_blastep_dy, I_ = _.__wbg_get_blastep_log2_min_a, j_ = _.__wbg_get_blastep_radius_alpha, M_ = _.__wbg_get_blastep_radius_beta, C_ = _.__wbg_get_incrementalunifiedbufferinfo_certificate_version, E_ = _.__wbg_get_incrementalunifiedbufferinfo_certificate_words_per_block, T_ = _.__wbg_get_incrementalunifiedbufferinfo_certificates_count, z_ = _.__wbg_get_incrementalunifiedbufferinfo_certificates_ptr, N_ = _.__wbg_get_incrementalunifiedbufferinfo_coeffs_count, B_ = _.__wbg_get_incrementalunifiedbufferinfo_coeffs_ptr, O_ = _.__wbg_get_incrementalunifiedbufferinfo_covered_orbit_len, D_ = _.__wbg_get_incrementalunifiedbufferinfo_cumulative_coefficients, L_ = _.__wbg_get_incrementalunifiedbufferinfo_cumulative_envelope_ms, U_ = _.__wbg_get_incrementalunifiedbufferinfo_cumulative_envelopes, $_ = _.__wbg_get_incrementalunifiedbufferinfo_cumulative_merge_coefficients_ms, P_ = _.__wbg_get_incrementalunifiedbufferinfo_cumulative_merges, W_ = _.__wbg_get_incrementalunifiedbufferinfo_has_more, G_ = _.__wbg_get_incrementalunifiedbufferinfo_peak_retained_bytes, V_ = _.__wbg_get_incrementalunifiedbufferinfo_published_orbit_len, H_ = _.__wbg_get_incrementalunifiedbufferinfo_radii_count, J_ = _.__wbg_get_incrementalunifiedbufferinfo_radii_ptr, K_ = _.__wbg_get_incrementalunifiedbufferinfo_range_count, Y_ = _.__wbg_get_incrementalunifiedbufferinfo_ranges_ptr, Q_ = _.__wbg_get_incrementalunifiedbufferinfo_reference_log2_dc, Z_ = _.__wbg_get_incrementalunifiedbufferinfo_reset, X_ = _.__wbg_get_incrementalunifiedrangeinfo_committed_count, q_ = _.__wbg_get_incrementalunifiedrangeinfo_payload_offset, et = _.__wbg_get_unifiedbufferinfo_validity_reference_log2_dc, _t = _.__wbg_get_unifiedbufferinfo_validity_version, tt = _.__wbg_get_unifiedbufferinfo_validity_words_per_block, nt = _.__wbg_incrementalunifiedbufferinfo_free, it = _.__wbg_incrementalunifiedrangeinfo_free, rt = _.__wbg_jetbufferinfo_free, ot = _.__wbg_mandelbrotnavigator_free, at = _.__wbg_mandelbrotstep_free, ft = _.__wbg_mobiusbufferinfo_free, st = _.__wbg_orbitbufferinfo_free, bt = _.__wbg_padebenchmark_free, ct = _.__wbg_set_blabufferinfo_count, lt = _.__wbg_set_blabufferinfo_level_count, gt = _.__wbg_set_blabufferinfo_levels_ptr, ut = _.__wbg_set_blabufferinfo_ptr, dt = _.__wbg_set_blastep_ab_exp, pt = _.__wbg_set_blastep_alpha_exp, wt = _.__wbg_set_blastep_ax, mt = _.__wbg_set_blastep_ay, vt = _.__wbg_set_blastep_bx, ht = _.__wbg_set_blastep_by, yt = _.__wbg_set_blastep_d_exp, kt = _.__wbg_set_blastep_dx, xt = _.__wbg_set_blastep_dy, Rt = _.__wbg_set_blastep_log2_min_a, Ft = _.__wbg_set_blastep_radius_alpha, St = _.__wbg_set_blastep_radius_beta, At = _.__wbg_set_incrementalunifiedbufferinfo_certificate_version, It = _.__wbg_set_incrementalunifiedbufferinfo_certificate_words_per_block, jt = _.__wbg_set_incrementalunifiedbufferinfo_certificates_count, Mt = _.__wbg_set_incrementalunifiedbufferinfo_certificates_ptr, Ct = _.__wbg_set_incrementalunifiedbufferinfo_coeffs_ptr, Et = _.__wbg_set_incrementalunifiedbufferinfo_covered_orbit_len, Tt = _.__wbg_set_incrementalunifiedbufferinfo_cumulative_coefficients, zt = _.__wbg_set_incrementalunifiedbufferinfo_cumulative_envelope_ms, Nt = _.__wbg_set_incrementalunifiedbufferinfo_cumulative_envelopes, Bt = _.__wbg_set_incrementalunifiedbufferinfo_cumulative_merge_coefficients_ms, Ot = _.__wbg_set_incrementalunifiedbufferinfo_cumulative_merges, Dt = _.__wbg_set_incrementalunifiedbufferinfo_has_more, Lt = _.__wbg_set_incrementalunifiedbufferinfo_peak_retained_bytes, Ut = _.__wbg_set_incrementalunifiedbufferinfo_published_orbit_len, $t = _.__wbg_set_incrementalunifiedbufferinfo_radii_count, Pt = _.__wbg_set_incrementalunifiedbufferinfo_radii_ptr, Wt = _.__wbg_set_incrementalunifiedbufferinfo_range_count, Gt = _.__wbg_set_incrementalunifiedbufferinfo_ranges_ptr, Vt = _.__wbg_set_incrementalunifiedbufferinfo_reference_log2_dc, Ht = _.__wbg_set_incrementalunifiedbufferinfo_reset, Jt = _.__wbg_set_incrementalunifiedrangeinfo_committed_count, Kt = _.__wbg_set_unifiedbufferinfo_validity_reference_log2_dc, Yt = _.__wbg_set_unifiedbufferinfo_validity_version, Qt = _.__wbg_set_unifiedbufferinfo_validity_words_per_block, Zt = _.__wbg_unifiedbufferinfo_free, Xt = _.mandelbrotnavigator_advance_incremental_unified_reference, qt = _.mandelbrotnavigator_angle, en = _.mandelbrotnavigator_begin_unified_reference, _n = _.mandelbrotnavigator_benchmark_pade, tn = _.mandelbrotnavigator_cancel_transition, nn = _.mandelbrotnavigator_compute_bla_reference_ptr, rn = _.mandelbrotnavigator_compute_jet_reference, on = _.mandelbrotnavigator_compute_mobius_reference, an = _.mandelbrotnavigator_compute_reference_orbit_chunk, fn = _.mandelbrotnavigator_compute_reference_orbit_ptr, sn = _.mandelbrotnavigator_compute_unified_header, bn = _.mandelbrotnavigator_compute_unified_reference, cn = _.mandelbrotnavigator_continue_unified_reference_bounds, ln = _.mandelbrotnavigator_coordinate_to_pixel, gn = _.mandelbrotnavigator_current_log2_c_max, un = _.mandelbrotnavigator_find_minibrot, dn = _.mandelbrotnavigator_find_minibrot_framed, pn = _.mandelbrotnavigator_finish_unified_reference, wn = _.mandelbrotnavigator_get_approximation_mode, mn = _.mandelbrotnavigator_get_bla_epsilon, vn = _.mandelbrotnavigator_get_dynamic_block_validity, hn = _.mandelbrotnavigator_get_gate_emission, yn = _.mandelbrotnavigator_get_incremental_reference_table, kn = _.mandelbrotnavigator_get_max_bla_skip, xn = _.mandelbrotnavigator_get_params, Rn = _.mandelbrotnavigator_get_reference_orbit_capacity, Fn = _.mandelbrotnavigator_get_reference_orbit_len, Sn = _.mandelbrotnavigator_get_reference_params, An = _.mandelbrotnavigator_is_in_transition, In = _.mandelbrotnavigator_new, jn = _.mandelbrotnavigator_origin, Mn = _.mandelbrotnavigator_pixel_to_complex, Cn = _.mandelbrotnavigator_reference_origin, En = _.mandelbrotnavigator_rotate, Tn = _.mandelbrotnavigator_rotate_direct, zn = _.mandelbrotnavigator_scale, Nn = _.mandelbrotnavigator_set_bla_epsilon, Bn = _.mandelbrotnavigator_set_dynamic_block_validity, On = _.mandelbrotnavigator_set_gate_emission, Dn = _.mandelbrotnavigator_set_incremental_reference_table, Ln = _.mandelbrotnavigator_set_max_bla_skip, Un = _.mandelbrotnavigator_set_precision_budget, $n = _.mandelbrotnavigator_set_viewport_aspect, Pn = _.mandelbrotnavigator_start_transition, Wn = _.mandelbrotnavigator_step, Gn = _.mandelbrotnavigator_translate, Vn = _.mandelbrotnavigator_translate_direct, Hn = _.mandelbrotnavigator_unified_is_cold, Jn = _.mandelbrotnavigator_unified_last_band_log2, Kn = _.mandelbrotnavigator_unified_last_band_spread, Yn = _.mandelbrotnavigator_unified_last_gate_count, Qn = _.mandelbrotnavigator_unified_last_periodic_detected_p, Zn = _.mandelbrotnavigator_unified_last_periodic_p, Xn = _.mandelbrotnavigator_unified_last_periodic_status, qn = _.mandelbrotnavigator_unified_last_sa_n0, ei = _.mandelbrotnavigator_unified_last_stages, _i = _.mandelbrotnavigator_use_bla, ti = _.mandelbrotnavigator_use_jet, ni = _.mandelbrotnavigator_use_mobius_cplus, ii = _.mandelbrotnavigator_use_pade, ri = _.mandelbrotnavigator_use_perturbation, oi = _.mandelbrotnavigator_use_unified, ai = _.mandelbrotnavigator_view_floatexp, fi = _.mandelbrotnavigator_zoom, si = _.__wbg_set_blalevel_count, bi = _.__wbg_set_blalevel_max_radius_bits, ci = _.__wbg_set_blalevel_offset, li = _.__wbg_set_blalevel_skip, gi = _.__wbg_set_incrementalunifiedbufferinfo_coeffs_count, ui = _.__wbg_set_incrementalunifiedrangeinfo_level, di = _.__wbg_set_incrementalunifiedrangeinfo_payload_offset, pi = _.__wbg_set_incrementalunifiedrangeinfo_skip, wi = _.__wbg_set_incrementalunifiedrangeinfo_slot_count, mi = _.__wbg_set_incrementalunifiedrangeinfo_slot_start, vi = _.__wbg_set_jetbufferinfo_coeffs_count, hi = _.__wbg_set_jetbufferinfo_coeffs_ptr, yi = _.__wbg_set_jetbufferinfo_level_count, ki = _.__wbg_set_jetbufferinfo_levels_ptr, xi = _.__wbg_set_jetbufferinfo_radii_count, Ri = _.__wbg_set_jetbufferinfo_radii_ptr, Fi = _.__wbg_set_mandelbrotstep_pad0, Si = _.__wbg_set_mandelbrotstep_pad1, Ai = _.__wbg_set_mandelbrotstep_zx, Ii = _.__wbg_set_mandelbrotstep_zy, ji = _.__wbg_set_mobiusbufferinfo_coeffs_count, Mi = _.__wbg_set_mobiusbufferinfo_coeffs_ptr, Ci = _.__wbg_set_mobiusbufferinfo_level_count, Ei = _.__wbg_set_mobiusbufferinfo_levels_ptr, Ti = _.__wbg_set_mobiusbufferinfo_radii_count, zi = _.__wbg_set_mobiusbufferinfo_radii_ptr, Ni = _.__wbg_set_orbitbufferinfo_count, Bi = _.__wbg_set_orbitbufferinfo_offset, Oi = _.__wbg_set_orbitbufferinfo_ptr, Di = _.__wbg_set_padebenchmark_max_iter, Li = _.__wbg_set_padebenchmark_max_iter_delta, Ui = _.__wbg_set_padebenchmark_pade_mismatches, $i = _.__wbg_set_padebenchmark_pixels, Pi = _.__wbg_set_padebenchmark_steps_affine, Wi = _.__wbg_set_padebenchmark_steps_exact, Gi = _.__wbg_set_padebenchmark_steps_pade, Vi = _.__wbg_set_unifiedbufferinfo_coeffs_count, Hi = _.__wbg_set_unifiedbufferinfo_coeffs_ptr, Ji = _.__wbg_set_unifiedbufferinfo_level_count, Ki = _.__wbg_set_unifiedbufferinfo_levels_ptr, Yi = _.__wbg_set_unifiedbufferinfo_optional_gate_log2_dc, Qi = _.__wbg_set_unifiedbufferinfo_optional_headers_count, Zi = _.__wbg_set_unifiedbufferinfo_optional_headers_ptr, Xi = _.__wbg_set_unifiedbufferinfo_optional_headers_version, qi = _.__wbg_set_unifiedbufferinfo_optional_periodic_log2_dc, er = _.__wbg_set_unifiedbufferinfo_optional_sa_log2_dc, _r = _.__wbg_set_unifiedbufferinfo_radii_count, tr = _.__wbg_set_unifiedbufferinfo_radii_ptr, nr = _.__wbg_set_unifiedbufferinfo_validity_count, ir = _.__wbg_set_unifiedbufferinfo_validity_diagnostics_count, rr = _.__wbg_set_unifiedbufferinfo_validity_diagnostics_ptr, or = _.__wbg_set_unifiedbufferinfo_validity_diagnostics_words_per_block, ar = _.__wbg_set_unifiedbufferinfo_validity_level_count, fr = _.__wbg_set_unifiedbufferinfo_validity_levels_ptr, sr = _.__wbg_set_unifiedbufferinfo_validity_ptr, br = _.__wbg_get_blalevel_count, cr = _.__wbg_get_blalevel_max_radius_bits, lr = _.__wbg_get_blalevel_offset, gr = _.__wbg_get_blalevel_skip, ur = _.__wbg_get_incrementalunifiedrangeinfo_level, dr = _.__wbg_get_incrementalunifiedrangeinfo_skip, pr = _.__wbg_get_incrementalunifiedrangeinfo_slot_count, wr = _.__wbg_get_incrementalunifiedrangeinfo_slot_start, mr = _.__wbg_get_jetbufferinfo_coeffs_count, vr = _.__wbg_get_jetbufferinfo_coeffs_ptr, hr = _.__wbg_get_jetbufferinfo_level_count, yr = _.__wbg_get_jetbufferinfo_levels_ptr, kr = _.__wbg_get_jetbufferinfo_radii_count, xr = _.__wbg_get_jetbufferinfo_radii_ptr, Rr = _.__wbg_get_mobiusbufferinfo_coeffs_count, Fr = _.__wbg_get_mobiusbufferinfo_coeffs_ptr, Sr = _.__wbg_get_mobiusbufferinfo_level_count, Ar = _.__wbg_get_mobiusbufferinfo_levels_ptr, Ir = _.__wbg_get_mobiusbufferinfo_radii_count, jr = _.__wbg_get_mobiusbufferinfo_radii_ptr, Mr = _.__wbg_get_orbitbufferinfo_count, Cr = _.__wbg_get_orbitbufferinfo_offset, Er = _.__wbg_get_orbitbufferinfo_ptr, Tr = _.__wbg_get_padebenchmark_max_iter, zr = _.__wbg_get_padebenchmark_max_iter_delta, Nr = _.__wbg_get_padebenchmark_pade_mismatches, Br = _.__wbg_get_padebenchmark_pixels, Or = _.__wbg_get_unifiedbufferinfo_coeffs_count, Dr = _.__wbg_get_unifiedbufferinfo_coeffs_ptr, Lr = _.__wbg_get_unifiedbufferinfo_level_count, Ur = _.__wbg_get_unifiedbufferinfo_levels_ptr, $r = _.__wbg_get_unifiedbufferinfo_optional_headers_count, Pr = _.__wbg_get_unifiedbufferinfo_optional_headers_ptr, Wr = _.__wbg_get_unifiedbufferinfo_optional_headers_version, Gr = _.__wbg_get_unifiedbufferinfo_radii_count, Vr = _.__wbg_get_unifiedbufferinfo_radii_ptr, Hr = _.__wbg_get_unifiedbufferinfo_validity_count, Jr = _.__wbg_get_unifiedbufferinfo_validity_diagnostics_count, Kr = _.__wbg_get_unifiedbufferinfo_validity_diagnostics_ptr, Yr = _.__wbg_get_unifiedbufferinfo_validity_diagnostics_words_per_block, Qr = _.__wbg_get_unifiedbufferinfo_validity_level_count, Zr = _.__wbg_get_unifiedbufferinfo_validity_levels_ptr, Xr = _.__wbg_get_unifiedbufferinfo_validity_ptr, qr = _.__wbg_get_mandelbrotstep_pad0, eo = _.__wbg_get_mandelbrotstep_pad1, _o = _.__wbg_get_mandelbrotstep_zx, to = _.__wbg_get_mandelbrotstep_zy, no = _.__wbg_get_padebenchmark_steps_affine, io = _.__wbg_get_padebenchmark_steps_exact, ro = _.__wbg_get_padebenchmark_steps_pade, oo = _.__wbg_get_unifiedbufferinfo_optional_gate_log2_dc, ao = _.__wbg_get_unifiedbufferinfo_optional_periodic_log2_dc, fo = _.__wbg_get_unifiedbufferinfo_optional_sa_log2_dc, so = _.__wbindgen_export_0, bo = _.__externref_drop_slice, co = _.__wbindgen_free, lo = _.__wbindgen_malloc, go = _.__wbindgen_realloc, He = _.__wbindgen_start;
    var uo = Object.freeze({
        __proto__: null,
        __externref_drop_slice: bo,
        __wbg_blabufferinfo_free: l_,
        __wbg_blalevel_free: g_,
        __wbg_blastep_free: u_,
        __wbg_get_blabufferinfo_count: d_,
        __wbg_get_blabufferinfo_level_count: p_,
        __wbg_get_blabufferinfo_levels_ptr: w_,
        __wbg_get_blabufferinfo_ptr: m_,
        __wbg_get_blalevel_count: br,
        __wbg_get_blalevel_max_radius_bits: cr,
        __wbg_get_blalevel_offset: lr,
        __wbg_get_blalevel_skip: gr,
        __wbg_get_blastep_ab_exp: v_,
        __wbg_get_blastep_alpha_exp: h_,
        __wbg_get_blastep_ax: y_,
        __wbg_get_blastep_ay: k_,
        __wbg_get_blastep_bx: x_,
        __wbg_get_blastep_by: R_,
        __wbg_get_blastep_d_exp: F_,
        __wbg_get_blastep_dx: S_,
        __wbg_get_blastep_dy: A_,
        __wbg_get_blastep_log2_min_a: I_,
        __wbg_get_blastep_radius_alpha: j_,
        __wbg_get_blastep_radius_beta: M_,
        __wbg_get_incrementalunifiedbufferinfo_certificate_version: C_,
        __wbg_get_incrementalunifiedbufferinfo_certificate_words_per_block: E_,
        __wbg_get_incrementalunifiedbufferinfo_certificates_count: T_,
        __wbg_get_incrementalunifiedbufferinfo_certificates_ptr: z_,
        __wbg_get_incrementalunifiedbufferinfo_coeffs_count: N_,
        __wbg_get_incrementalunifiedbufferinfo_coeffs_ptr: B_,
        __wbg_get_incrementalunifiedbufferinfo_covered_orbit_len: O_,
        __wbg_get_incrementalunifiedbufferinfo_cumulative_coefficients: D_,
        __wbg_get_incrementalunifiedbufferinfo_cumulative_envelope_ms: L_,
        __wbg_get_incrementalunifiedbufferinfo_cumulative_envelopes: U_,
        __wbg_get_incrementalunifiedbufferinfo_cumulative_merge_coefficients_ms: $_,
        __wbg_get_incrementalunifiedbufferinfo_cumulative_merges: P_,
        __wbg_get_incrementalunifiedbufferinfo_has_more: W_,
        __wbg_get_incrementalunifiedbufferinfo_peak_retained_bytes: G_,
        __wbg_get_incrementalunifiedbufferinfo_published_orbit_len: V_,
        __wbg_get_incrementalunifiedbufferinfo_radii_count: H_,
        __wbg_get_incrementalunifiedbufferinfo_radii_ptr: J_,
        __wbg_get_incrementalunifiedbufferinfo_range_count: K_,
        __wbg_get_incrementalunifiedbufferinfo_ranges_ptr: Y_,
        __wbg_get_incrementalunifiedbufferinfo_reference_log2_dc: Q_,
        __wbg_get_incrementalunifiedbufferinfo_reset: Z_,
        __wbg_get_incrementalunifiedrangeinfo_committed_count: X_,
        __wbg_get_incrementalunifiedrangeinfo_level: ur,
        __wbg_get_incrementalunifiedrangeinfo_payload_offset: q_,
        __wbg_get_incrementalunifiedrangeinfo_skip: dr,
        __wbg_get_incrementalunifiedrangeinfo_slot_count: pr,
        __wbg_get_incrementalunifiedrangeinfo_slot_start: wr,
        __wbg_get_jetbufferinfo_coeffs_count: mr,
        __wbg_get_jetbufferinfo_coeffs_ptr: vr,
        __wbg_get_jetbufferinfo_level_count: hr,
        __wbg_get_jetbufferinfo_levels_ptr: yr,
        __wbg_get_jetbufferinfo_radii_count: kr,
        __wbg_get_jetbufferinfo_radii_ptr: xr,
        __wbg_get_mandelbrotstep_pad0: qr,
        __wbg_get_mandelbrotstep_pad1: eo,
        __wbg_get_mandelbrotstep_zx: _o,
        __wbg_get_mandelbrotstep_zy: to,
        __wbg_get_mobiusbufferinfo_coeffs_count: Rr,
        __wbg_get_mobiusbufferinfo_coeffs_ptr: Fr,
        __wbg_get_mobiusbufferinfo_level_count: Sr,
        __wbg_get_mobiusbufferinfo_levels_ptr: Ar,
        __wbg_get_mobiusbufferinfo_radii_count: Ir,
        __wbg_get_mobiusbufferinfo_radii_ptr: jr,
        __wbg_get_orbitbufferinfo_count: Mr,
        __wbg_get_orbitbufferinfo_offset: Cr,
        __wbg_get_orbitbufferinfo_ptr: Er,
        __wbg_get_padebenchmark_max_iter: Tr,
        __wbg_get_padebenchmark_max_iter_delta: zr,
        __wbg_get_padebenchmark_pade_mismatches: Nr,
        __wbg_get_padebenchmark_pixels: Br,
        __wbg_get_padebenchmark_steps_affine: no,
        __wbg_get_padebenchmark_steps_exact: io,
        __wbg_get_padebenchmark_steps_pade: ro,
        __wbg_get_unifiedbufferinfo_coeffs_count: Or,
        __wbg_get_unifiedbufferinfo_coeffs_ptr: Dr,
        __wbg_get_unifiedbufferinfo_level_count: Lr,
        __wbg_get_unifiedbufferinfo_levels_ptr: Ur,
        __wbg_get_unifiedbufferinfo_optional_gate_log2_dc: oo,
        __wbg_get_unifiedbufferinfo_optional_headers_count: $r,
        __wbg_get_unifiedbufferinfo_optional_headers_ptr: Pr,
        __wbg_get_unifiedbufferinfo_optional_headers_version: Wr,
        __wbg_get_unifiedbufferinfo_optional_periodic_log2_dc: ao,
        __wbg_get_unifiedbufferinfo_optional_sa_log2_dc: fo,
        __wbg_get_unifiedbufferinfo_radii_count: Gr,
        __wbg_get_unifiedbufferinfo_radii_ptr: Vr,
        __wbg_get_unifiedbufferinfo_validity_count: Hr,
        __wbg_get_unifiedbufferinfo_validity_diagnostics_count: Jr,
        __wbg_get_unifiedbufferinfo_validity_diagnostics_ptr: Kr,
        __wbg_get_unifiedbufferinfo_validity_diagnostics_words_per_block: Yr,
        __wbg_get_unifiedbufferinfo_validity_level_count: Qr,
        __wbg_get_unifiedbufferinfo_validity_levels_ptr: Zr,
        __wbg_get_unifiedbufferinfo_validity_ptr: Xr,
        __wbg_get_unifiedbufferinfo_validity_reference_log2_dc: et,
        __wbg_get_unifiedbufferinfo_validity_version: _t,
        __wbg_get_unifiedbufferinfo_validity_words_per_block: tt,
        __wbg_incrementalunifiedbufferinfo_free: nt,
        __wbg_incrementalunifiedrangeinfo_free: it,
        __wbg_jetbufferinfo_free: rt,
        __wbg_mandelbrotnavigator_free: ot,
        __wbg_mandelbrotstep_free: at,
        __wbg_mobiusbufferinfo_free: ft,
        __wbg_orbitbufferinfo_free: st,
        __wbg_padebenchmark_free: bt,
        __wbg_set_blabufferinfo_count: ct,
        __wbg_set_blabufferinfo_level_count: lt,
        __wbg_set_blabufferinfo_levels_ptr: gt,
        __wbg_set_blabufferinfo_ptr: ut,
        __wbg_set_blalevel_count: si,
        __wbg_set_blalevel_max_radius_bits: bi,
        __wbg_set_blalevel_offset: ci,
        __wbg_set_blalevel_skip: li,
        __wbg_set_blastep_ab_exp: dt,
        __wbg_set_blastep_alpha_exp: pt,
        __wbg_set_blastep_ax: wt,
        __wbg_set_blastep_ay: mt,
        __wbg_set_blastep_bx: vt,
        __wbg_set_blastep_by: ht,
        __wbg_set_blastep_d_exp: yt,
        __wbg_set_blastep_dx: kt,
        __wbg_set_blastep_dy: xt,
        __wbg_set_blastep_log2_min_a: Rt,
        __wbg_set_blastep_radius_alpha: Ft,
        __wbg_set_blastep_radius_beta: St,
        __wbg_set_incrementalunifiedbufferinfo_certificate_version: At,
        __wbg_set_incrementalunifiedbufferinfo_certificate_words_per_block: It,
        __wbg_set_incrementalunifiedbufferinfo_certificates_count: jt,
        __wbg_set_incrementalunifiedbufferinfo_certificates_ptr: Mt,
        __wbg_set_incrementalunifiedbufferinfo_coeffs_count: gi,
        __wbg_set_incrementalunifiedbufferinfo_coeffs_ptr: Ct,
        __wbg_set_incrementalunifiedbufferinfo_covered_orbit_len: Et,
        __wbg_set_incrementalunifiedbufferinfo_cumulative_coefficients: Tt,
        __wbg_set_incrementalunifiedbufferinfo_cumulative_envelope_ms: zt,
        __wbg_set_incrementalunifiedbufferinfo_cumulative_envelopes: Nt,
        __wbg_set_incrementalunifiedbufferinfo_cumulative_merge_coefficients_ms: Bt,
        __wbg_set_incrementalunifiedbufferinfo_cumulative_merges: Ot,
        __wbg_set_incrementalunifiedbufferinfo_has_more: Dt,
        __wbg_set_incrementalunifiedbufferinfo_peak_retained_bytes: Lt,
        __wbg_set_incrementalunifiedbufferinfo_published_orbit_len: Ut,
        __wbg_set_incrementalunifiedbufferinfo_radii_count: $t,
        __wbg_set_incrementalunifiedbufferinfo_radii_ptr: Pt,
        __wbg_set_incrementalunifiedbufferinfo_range_count: Wt,
        __wbg_set_incrementalunifiedbufferinfo_ranges_ptr: Gt,
        __wbg_set_incrementalunifiedbufferinfo_reference_log2_dc: Vt,
        __wbg_set_incrementalunifiedbufferinfo_reset: Ht,
        __wbg_set_incrementalunifiedrangeinfo_committed_count: Jt,
        __wbg_set_incrementalunifiedrangeinfo_level: ui,
        __wbg_set_incrementalunifiedrangeinfo_payload_offset: di,
        __wbg_set_incrementalunifiedrangeinfo_skip: pi,
        __wbg_set_incrementalunifiedrangeinfo_slot_count: wi,
        __wbg_set_incrementalunifiedrangeinfo_slot_start: mi,
        __wbg_set_jetbufferinfo_coeffs_count: vi,
        __wbg_set_jetbufferinfo_coeffs_ptr: hi,
        __wbg_set_jetbufferinfo_level_count: yi,
        __wbg_set_jetbufferinfo_levels_ptr: ki,
        __wbg_set_jetbufferinfo_radii_count: xi,
        __wbg_set_jetbufferinfo_radii_ptr: Ri,
        __wbg_set_mandelbrotstep_pad0: Fi,
        __wbg_set_mandelbrotstep_pad1: Si,
        __wbg_set_mandelbrotstep_zx: Ai,
        __wbg_set_mandelbrotstep_zy: Ii,
        __wbg_set_mobiusbufferinfo_coeffs_count: ji,
        __wbg_set_mobiusbufferinfo_coeffs_ptr: Mi,
        __wbg_set_mobiusbufferinfo_level_count: Ci,
        __wbg_set_mobiusbufferinfo_levels_ptr: Ei,
        __wbg_set_mobiusbufferinfo_radii_count: Ti,
        __wbg_set_mobiusbufferinfo_radii_ptr: zi,
        __wbg_set_orbitbufferinfo_count: Ni,
        __wbg_set_orbitbufferinfo_offset: Bi,
        __wbg_set_orbitbufferinfo_ptr: Oi,
        __wbg_set_padebenchmark_max_iter: Di,
        __wbg_set_padebenchmark_max_iter_delta: Li,
        __wbg_set_padebenchmark_pade_mismatches: Ui,
        __wbg_set_padebenchmark_pixels: $i,
        __wbg_set_padebenchmark_steps_affine: Pi,
        __wbg_set_padebenchmark_steps_exact: Wi,
        __wbg_set_padebenchmark_steps_pade: Gi,
        __wbg_set_unifiedbufferinfo_coeffs_count: Vi,
        __wbg_set_unifiedbufferinfo_coeffs_ptr: Hi,
        __wbg_set_unifiedbufferinfo_level_count: Ji,
        __wbg_set_unifiedbufferinfo_levels_ptr: Ki,
        __wbg_set_unifiedbufferinfo_optional_gate_log2_dc: Yi,
        __wbg_set_unifiedbufferinfo_optional_headers_count: Qi,
        __wbg_set_unifiedbufferinfo_optional_headers_ptr: Zi,
        __wbg_set_unifiedbufferinfo_optional_headers_version: Xi,
        __wbg_set_unifiedbufferinfo_optional_periodic_log2_dc: qi,
        __wbg_set_unifiedbufferinfo_optional_sa_log2_dc: er,
        __wbg_set_unifiedbufferinfo_radii_count: _r,
        __wbg_set_unifiedbufferinfo_radii_ptr: tr,
        __wbg_set_unifiedbufferinfo_validity_count: nr,
        __wbg_set_unifiedbufferinfo_validity_diagnostics_count: ir,
        __wbg_set_unifiedbufferinfo_validity_diagnostics_ptr: rr,
        __wbg_set_unifiedbufferinfo_validity_diagnostics_words_per_block: or,
        __wbg_set_unifiedbufferinfo_validity_level_count: ar,
        __wbg_set_unifiedbufferinfo_validity_levels_ptr: fr,
        __wbg_set_unifiedbufferinfo_validity_ptr: sr,
        __wbg_set_unifiedbufferinfo_validity_reference_log2_dc: Kt,
        __wbg_set_unifiedbufferinfo_validity_version: Yt,
        __wbg_set_unifiedbufferinfo_validity_words_per_block: Qt,
        __wbg_unifiedbufferinfo_free: Zt,
        __wbindgen_export_0: so,
        __wbindgen_free: co,
        __wbindgen_malloc: lo,
        __wbindgen_realloc: go,
        __wbindgen_start: He,
        mandelbrotnavigator_advance_incremental_unified_reference: Xt,
        mandelbrotnavigator_angle: qt,
        mandelbrotnavigator_begin_unified_reference: en,
        mandelbrotnavigator_benchmark_pade: _n,
        mandelbrotnavigator_cancel_transition: tn,
        mandelbrotnavigator_compute_bla_reference_ptr: nn,
        mandelbrotnavigator_compute_jet_reference: rn,
        mandelbrotnavigator_compute_mobius_reference: on,
        mandelbrotnavigator_compute_reference_orbit_chunk: an,
        mandelbrotnavigator_compute_reference_orbit_ptr: fn,
        mandelbrotnavigator_compute_unified_header: sn,
        mandelbrotnavigator_compute_unified_reference: bn,
        mandelbrotnavigator_continue_unified_reference_bounds: cn,
        mandelbrotnavigator_coordinate_to_pixel: ln,
        mandelbrotnavigator_current_log2_c_max: gn,
        mandelbrotnavigator_find_minibrot: un,
        mandelbrotnavigator_find_minibrot_framed: dn,
        mandelbrotnavigator_finish_unified_reference: pn,
        mandelbrotnavigator_get_approximation_mode: wn,
        mandelbrotnavigator_get_bla_epsilon: mn,
        mandelbrotnavigator_get_dynamic_block_validity: vn,
        mandelbrotnavigator_get_gate_emission: hn,
        mandelbrotnavigator_get_incremental_reference_table: yn,
        mandelbrotnavigator_get_max_bla_skip: kn,
        mandelbrotnavigator_get_params: xn,
        mandelbrotnavigator_get_reference_orbit_capacity: Rn,
        mandelbrotnavigator_get_reference_orbit_len: Fn,
        mandelbrotnavigator_get_reference_params: Sn,
        mandelbrotnavigator_is_in_transition: An,
        mandelbrotnavigator_new: In,
        mandelbrotnavigator_origin: jn,
        mandelbrotnavigator_pixel_to_complex: Mn,
        mandelbrotnavigator_reference_origin: Cn,
        mandelbrotnavigator_rotate: En,
        mandelbrotnavigator_rotate_direct: Tn,
        mandelbrotnavigator_scale: zn,
        mandelbrotnavigator_set_bla_epsilon: Nn,
        mandelbrotnavigator_set_dynamic_block_validity: Bn,
        mandelbrotnavigator_set_gate_emission: On,
        mandelbrotnavigator_set_incremental_reference_table: Dn,
        mandelbrotnavigator_set_max_bla_skip: Ln,
        mandelbrotnavigator_set_precision_budget: Un,
        mandelbrotnavigator_set_viewport_aspect: $n,
        mandelbrotnavigator_start_transition: Pn,
        mandelbrotnavigator_step: Wn,
        mandelbrotnavigator_translate: Gn,
        mandelbrotnavigator_translate_direct: Vn,
        mandelbrotnavigator_unified_is_cold: Hn,
        mandelbrotnavigator_unified_last_band_log2: Jn,
        mandelbrotnavigator_unified_last_band_spread: Kn,
        mandelbrotnavigator_unified_last_gate_count: Yn,
        mandelbrotnavigator_unified_last_periodic_detected_p: Qn,
        mandelbrotnavigator_unified_last_periodic_p: Zn,
        mandelbrotnavigator_unified_last_periodic_status: Xn,
        mandelbrotnavigator_unified_last_sa_n0: qn,
        mandelbrotnavigator_unified_last_stages: ei,
        mandelbrotnavigator_use_bla: _i,
        mandelbrotnavigator_use_jet: ti,
        mandelbrotnavigator_use_mobius_cplus: ni,
        mandelbrotnavigator_use_pade: ii,
        mandelbrotnavigator_use_perturbation: ri,
        mandelbrotnavigator_use_unified: oi,
        mandelbrotnavigator_view_floatexp: ai,
        mandelbrotnavigator_zoom: fi,
        memory: y
    });
    __(uo);
    He();
    const po = 3, $e = 21, wo = 6, Pe = 27, mo = 4;
    function vo(n, e) {
        return n.jobId === e.jobId && n.refId === e.refId && n.tableGeneration === e.tableGeneration;
    }
    function ho(n) {
        return n !== "viewport-update";
    }
    function yo(n, e) {
        return n ? "epoch-reset" : e > 0 ? "reference-growth" : "none";
    }
    function ko(n) {
        if (n.version !== po || n.wordsPerBlock !== $e || n.rangesWords % wo !== 0 || n.coefficientFloats % Pe !== 0) return !1;
        const e = n.coefficientFloats / Pe;
        return n.sidecarFloats === e * mo && n.certificateWords === e * $e;
    }
    const Re = self;
    let f, h = 0, Y = !1, d = 0, R = !1, p = 0, G = 0, ye = !1, J = !1, xo = 0, D = 0, _e = Number.NaN, ue = -1, de = -1, Ro = 0, V = 0, H = 0, Fe = 0, Je = 0, Se = "none", F = "none", I = "";
    const Ae = 50, Fo = 2, We = 1e7, So = Ae, Ao = Ae;
    function S(n, e) {
        Re.postMessage(n, e ?? []);
    }
    function B() {
        return new Promise((n)=>setTimeout(n, 0));
    }
    function Ke(n, e) {
        const i = e instanceof Error ? e.message : String(e);
        S({
            type: "error",
            jobId: n,
            message: i
        });
    }
    function Ye(n) {
        f && (n === "bla" ? f.use_bla() : n === "pade" ? f.use_pade() : n === "jet" ? f.use_jet() : n === "mobius" ? f.use_mobius_cplus() : n === "auto" ? f.use_unified() : f.use_perturbation());
    }
    function Io(n) {
        console.log("[REF worker] RESET (fresh navigator)", n.cx.slice(0, 14), "scale", n.scale.slice(0, 10)), f?.free(), f = new xe(n.cx, n.cy, n.scale, n.angle), f.set_precision_budget(n.precisionBudget), h = n.jobId, d = 0, R = !1, p = n.tableGeneration ?? 0, G = n.maxIterations, J = !1, Ye(n.approximationMode), f.set_bla_epsilon(n.blaEpsilon), f.set_gate_emission(!!n.gateEmission), f.set_dynamic_block_validity(!!n.dynamicBlockValidity), f.set_incremental_reference_table(!!n.incrementalReferenceTable), f.set_max_bla_skip(n.maxBlaSkip), f.set_viewport_aspect(n.viewportAspect ?? Number.NaN), _e = f.current_log2_c_max(), ue = -1, de = -1, V = 0, H = 0, Fe = 0, Je = 0, Se = "none", F = "epoch-reset", I = "", C(n.jobId);
    }
    function jo(n, e, i) {
        const s = Math.max(0, i - e), o = new Float32Array(y.buffer, n + e * 4 * Float32Array.BYTES_PER_ELEMENT, s * 4), c = new Float32Array(s * 2);
        for(let l = 0; l < s; l++)c[l * 2] = o[l * 4], c[l * 2 + 1] = o[l * 4 + 1];
        return c;
    }
    function Qe(n, e) {
        if (n.optional_headers_count <= 0) return;
        if (n.optional_headers_version <= 0 || n.optional_headers_count < 11 || Number.isNaN(n.optional_sa_log2_dc) || Number.isNaN(n.optional_periodic_log2_dc) || Number.isNaN(n.optional_gate_log2_dc) || !Number.isFinite(e)) throw new Error(`invalid optional-header contract: version=${n.optional_headers_version} records=${n.optional_headers_count} domains=${n.optional_sa_log2_dc}/${n.optional_periodic_log2_dc}/${n.optional_gate_log2_dc}`);
        const i = new Float32Array(y.buffer, n.optional_headers_ptr, n.optional_headers_count * 4), r = new Float32Array(i.length);
        return r.set(i), {
            version: n.optional_headers_version,
            revision: ++Ro,
            currentLog2CMax: e,
            saLog2Dc: n.optional_sa_log2_dc,
            periodicLog2Dc: n.optional_periodic_log2_dc,
            gateLog2Dc: n.optional_gate_log2_dc,
            data: r
        };
    }
    function Mo(n) {
        let e = 1;
        const i = Math.max(1, Math.ceil(n));
        for(; e < i;)e *= 2;
        return e;
    }
    function pe(n, e, i, r) {
        return !Y && f === n && vo({
            jobId: h,
            refId: D,
            tableGeneration: p
        }, {
            jobId: e,
            refId: i,
            tableGeneration: r
        });
    }
    function ce(n, e) {
        const i = f;
        if (!i || i.get_approximation_mode() !== 5 || !i.get_dynamic_block_validity() || !i.get_incremental_reference_table()) return {
            hasMore: !1,
            published: !1
        };
        const r = D, a = p, s = i.current_log2_c_max();
        if (!pe(i, n, r, a)) return H++, {
            hasMore: !1,
            published: !1
        };
        const o = i.advance_incremental_unified_reference(e, So, Ao);
        try {
            if (!pe(i, n, r, a)) return H++, {
                hasMore: !1,
                published: !1
            };
            const c = new Uint32Array(y.buffer, o.ranges_ptr, o.range_count * 6), l = new Uint32Array(c), g = new Float32Array(y.buffer, o.coeffs_ptr, o.coeffs_count * 27), w = new Float32Array(g), u = new Float32Array(y.buffer, o.radii_ptr, o.radii_count * 4), Q = new Float32Array(u);
            if (!ko({
                version: o.certificate_version,
                wordsPerBlock: o.certificate_words_per_block,
                rangesWords: l.length,
                coefficientFloats: w.length,
                sidecarFloats: Q.length,
                certificateWords: o.certificates_count * o.certificate_words_per_block,
                referenceLog2Dc: o.reference_log2_dc
            }) || o.coeffs_count !== o.certificates_count) throw new Error(`incremental radial payload mismatch version=${o.certificate_version} words=${o.certificate_words_per_block} records=${o.coeffs_count}/${o.radii_count}/${o.certificates_count} domain=${o.reference_log2_dc}`);
            const A = new Uint32Array(y.buffer, o.certificates_ptr, o.certificates_count * o.certificate_words_per_block), fe = new Uint32Array(A);
            if (!pe(i, n, r, a)) return H++, {
                hasMore: !1,
                published: !1
            };
            const z = o.has_more !== 0 || o.covered_orbit_len < e + 1, L = l.length > 0 || o.reset !== 0;
            if (L) {
                const Z = yo(o.reset !== 0, o.certificates_count), b = o.reset !== 0 ? "epoch-reset" : F === "none" ? Z : F;
                o.certificates_count > 0 && b === "reference-growth" && (Fe += o.certificates_count), b !== "none" && (Se = b);
                const N = Math.max(e, Math.max(0, o.covered_orbit_len - 1));
                S({
                    type: "tableRange",
                    jobId: n,
                    refId: r,
                    tableGeneration: a,
                    maxIterations: N,
                    capacityOrbitLength: Mo(Math.max(1024, N + 1)),
                    coveredOrbitLength: o.published_orbit_len,
                    builtOrbitLength: o.covered_orbit_len,
                    reset: o.reset !== 0,
                    hasMore: z,
                    ranges: l,
                    coefficients: w,
                    radii: Q,
                    certificates: fe,
                    certificateVersion: o.certificate_version,
                    certificateWordsPerBlock: o.certificate_words_per_block,
                    referenceLog2Dc: o.reference_log2_dc,
                    currentLog2CMax: s,
                    cumulativeMerges: o.cumulative_merges,
                    cumulativeCoefficients: o.cumulative_coefficients,
                    cumulativeCertificates: o.cumulative_envelopes,
                    peakRetainedBytes: o.peak_retained_bytes,
                    cumulativeMergeCoefficientsMs: o.cumulative_merge_coefficients_ms,
                    cumulativeCertificateMs: o.cumulative_envelope_ms,
                    referenceGrowthCertificates: Fe,
                    viewportOnlyCertificateBuilds: Je,
                    lastCertificateBuildCause: Se,
                    yields: V,
                    cancellations: H
                }, [
                    l.buffer,
                    w.buffer,
                    Q.buffer,
                    fe.buffer
                ]);
            }
            return z || (F = "none"), {
                hasMore: z,
                published: L
            };
        } finally{
            o.free();
        }
    }
    function Co(n, e) {
        const i = f;
        if (!i) return;
        const r = D, a = p, s = i.current_log2_c_max(), o = `${n}/${r}/${a}/${e}/${s}`;
        if (o === I) return;
        const c = performance.now(), l = i.compute_unified_header(e);
        try {
            if (!pe(i, n, r, a)) {
                H++;
                return;
            }
            const g = Qe(l, s);
            if (!g) return;
            I = o, S({
                type: "headersReady",
                jobId: n,
                refId: r,
                maxIterations: e,
                optionalHeaders: g,
                buildMs: performance.now() - c,
                buildStages: 16,
                tableStats: {
                    coefficientsMs: 0,
                    boundsMs: 0,
                    radiiMs: 0,
                    saN0: i.unified_last_sa_n0(),
                    periodicP: i.unified_last_periodic_p(),
                    periodicStatus: i.unified_last_periodic_status(),
                    periodicDetectedP: i.unified_last_periodic_detected_p(),
                    bandLog2: Number.NaN,
                    bandSpread: Number.NaN,
                    gateCount: 0
                },
                tableGeneration: a
            }, [
                g.data.buffer
            ]);
        } finally{
            l.free();
        }
    }
    function Ge(n, e, i) {
        if (!f || n !== h || Y) return;
        const r = f.get_approximation_mode(), a = (r === 3 || r === 4 || r === 5) && d > 0 && e <= Math.ceil(d * 1.5), s = d >= e || a, o = R && (r === 3 || r === 4 || r === 5), c = s ? d : Math.max(d, e);
        if (s && !o || i < c || r === 0) return;
        const l = D, g = r === 3, w = r === 4, u = r === 5, Q = u ? "unified" : w ? "mobius" : g ? "jet" : "bla", A = (x, M)=>{
            S({
                type: "tableProgress",
                jobId: n,
                refId: l,
                tableGeneration: p,
                kind: Q,
                progress: x,
                stage: M
            });
        };
        if (!g && !w && !u) {
            A(0, "coefficients");
            const x = f.compute_bla_reference_ptr(c);
            A(.9, "transfer");
            const M = new Float32Array(y.buffer, x.ptr, x.count * 12), E = new Float32Array(M.length);
            E.set(M);
            const ee = new Uint32Array(y.buffer, x.levels_ptr, x.level_count * 4), $ = new Uint32Array(ee.length);
            $.set(ee), d = c, S({
                type: "blaReady",
                jobId: n,
                refId: l,
                maxIterations: c,
                kind: "bla",
                steps: E,
                levels: $,
                levelCount: x.level_count,
                tableGeneration: p
            }, [
                E.buffer,
                $.buffer
            ]);
            return;
        }
        const fe = performance.now();
        let z = 0, L = 0, Z = 0, b;
        if (u) {
            const x = performance.now();
            f.begin_unified_reference(c), z = performance.now() - x;
            const M = performance.now();
            f.continue_unified_reference_bounds(c), L = performance.now() - M;
            const E = performance.now();
            b = f.finish_unified_reference(c), Z = performance.now() - E;
        } else A(0, "coefficients"), b = w ? f.compute_mobius_reference(c) : f.compute_jet_reference(c);
        const N = performance.now() - fe, m = u ? f.unified_last_stages() : void 0;
        u && m !== void 0 ? ((m & 1) !== 0 && A(1 / 3, "coefficients"), (m & 2) !== 0 && A(2 / 3, "bounds"), (m & 12) !== 0 && A(.85, "radii"), (m & 15) !== 0 && A(.9, "transfer"), (m & 1) === 0 && (z = 0), (m & 2) === 0 && (L = 0), (m & 12) === 0 && (Z = 0)) : A(.9, "transfer");
        const we = u ? {
            coefficientsMs: z,
            boundsMs: L,
            radiiMs: Z,
            saN0: f.unified_last_sa_n0(),
            periodicP: f.unified_last_periodic_p(),
            periodicStatus: f.unified_last_periodic_status(),
            periodicDetectedP: f.unified_last_periodic_detected_p(),
            bandLog2: f.unified_last_band_log2(),
            bandSpread: f.unified_last_band_spread(),
            gateCount: f.unified_last_gate_count()
        } : void 0;
        console.log(`[REF worker] ${w ? "mobius" : u ? "unified" : "jet"} table built in ${N.toFixed(0)}ms (maxIter ${c}${m !== void 0 ? `, stages ${m}` : ""})`);
        const j = u ? Qe(b, f.current_log2_c_max()) : void 0;
        if (u && !j) throw new Error("unified table omitted its mandatory optional-header payload");
        if (d = Math.max(d, c), R = !1, u && f.get_dynamic_block_validity() && m === 16 && ue === l && de === p) {
            S({
                type: "headersReady",
                jobId: n,
                refId: l,
                maxIterations: c,
                optionalHeaders: j,
                buildMs: N,
                buildStages: m,
                tableStats: we,
                tableGeneration: p
            }, [
                j.data.buffer
            ]);
            return;
        }
        const Ie = new Float32Array(y.buffer, b.radii_ptr, b.radii_count * 4), X = new Float32Array(Ie.length);
        X.set(Ie);
        const je = new Uint32Array(y.buffer, b.levels_ptr, b.level_count * 4), q = new Uint32Array(je.length);
        if (q.set(je), u && m !== void 0 && (m & 4) !== 0 && (m & -21) === 0 && ue === l && de === p) {
            S({
                type: "radiiReady",
                jobId: n,
                refId: l,
                maxIterations: c,
                radii: X,
                optionalHeaders: j,
                levels: q,
                levelCount: b.level_count,
                buildMs: N,
                buildStages: m,
                tableStats: we,
                tableGeneration: p
            }, [
                X.buffer,
                q.buffer,
                ...j ? [
                    j.data.buffer
                ] : []
            ]);
            return;
        }
        const Ze = w ? 21 : 27, Me = new Float32Array(y.buffer, b.coeffs_ptr, b.coeffs_count * Ze), me = new Float32Array(Me.length);
        me.set(Me);
        let U;
        if (u && b.validity_count > 0) {
            if (b.validity_version <= 0 || b.validity_words_per_block <= 0 || b.validity_diagnostics_words_per_block <= 0 || b.validity_count !== b.coeffs_count || b.validity_diagnostics_count !== b.validity_count || b.validity_level_count !== b.level_count || !Number.isFinite(b.validity_reference_log2_dc)) throw new Error(`invalid dynamic-validity buffer contract: version=${b.validity_version} words=${b.validity_words_per_block} records=${b.validity_count}/${b.coeffs_count} diagnostics=${b.validity_diagnostics_words_per_block}x${b.validity_diagnostics_count} levels=${b.validity_level_count}/${b.level_count} domain=${b.validity_reference_log2_dc}`);
            const x = new Float32Array(y.buffer, b.validity_ptr, b.validity_count * b.validity_words_per_block), M = new Float32Array(x.length);
            M.set(x);
            const E = new Uint32Array(y.buffer, b.validity_diagnostics_ptr, b.validity_diagnostics_count * b.validity_diagnostics_words_per_block), ee = new Uint32Array(E.length);
            ee.set(E);
            const $ = new Uint32Array(y.buffer, b.validity_levels_ptr, b.validity_level_count * 4), Ce = new Uint32Array($.length);
            Ce.set($), U = {
                version: b.validity_version,
                wordsPerBlock: b.validity_words_per_block,
                diagnosticsWordsPerBlock: b.validity_diagnostics_words_per_block,
                referenceLog2Dc: b.validity_reference_log2_dc,
                envelopes: M,
                diagnostics: ee,
                levels: Ce,
                levelCount: b.validity_level_count
            };
        }
        u && (ue = l, de = p);
        const Xe = {
            type: "blaReady",
            jobId: n,
            refId: l,
            maxIterations: c,
            kind: w ? "mobius" : u ? "unified" : "jet",
            steps: me,
            radii: X,
            optionalHeaders: j,
            validity: U,
            levels: q,
            levelCount: b.level_count,
            buildMs: N,
            buildStages: m,
            tableStats: we,
            tableGeneration: p
        }, ve = [
            me.buffer,
            X.buffer,
            q.buffer
        ];
        j && ve.push(j.data.buffer), U && ve.push(U.envelopes.buffer, U.diagnostics.buffer, U.levels.buffer), S(Xe, ve);
    }
    function ke(n, e, i) {
        if (!f) return 0;
        const r = f.compute_reference_orbit_chunk(Ae, i);
        J = !1;
        const a = jo(r.ptr, r.offset, r.count), [s, o] = f.get_reference_params();
        r.offset === 0 ? (F = "epoch-reset", D = ++xo, d = 0, R = !1, I = "", console.log("[REF worker] orbit (re)start refId=", D, "ref=", s.slice(0, 14))) : F === "none" && (F = "reference-growth");
        const c = Math.max(0, r.count - 1);
        return S({
            type: "orbitChunk",
            jobId: n,
            refId: D,
            offset: r.offset,
            count: r.count,
            maxIterations: e,
            referenceCx: s,
            referenceCy: o,
            orbit: a
        }, [
            a.buffer
        ]), c;
    }
    async function C(n) {
        if (ye) return;
        ye = !0;
        let e = !1;
        try {
            for(; !Y && f && n === h;){
                const i = G, r = Math.min(i, We), a = Math.min(i * Fo, We), s = Math.max(0, f.get_reference_orbit_len()), o = f.get_approximation_mode() === 5 && f.get_dynamic_block_validity() && f.get_incremental_reference_table();
                if (J || s < r) {
                    const l = ke(n, i, r);
                    o && l > 0 && ce(n, a), V += o ? 1 : 0, await B();
                    continue;
                }
                if (o) {
                    const l = ce(n, i);
                    if (l.published || l.hasMore) {
                        V++, await B();
                        continue;
                    }
                    if (d = Math.max(d, i), s < a) {
                        ke(n, i, a) > 0 && ce(n, a), V++, await B();
                        continue;
                    }
                    const g = ce(n, Math.min(s, a));
                    if (g.published || g.hasMore) {
                        V++, await B();
                        continue;
                    }
                    if (Co(n, i), await B(), G <= i && !J) break;
                    continue;
                }
                if (s >= a) {
                    if (Ge(n, i, s), await B(), G <= i) break;
                    continue;
                }
                const c = ke(n, i, a);
                Ge(n, i, c), await B();
            }
        } catch (i) {
            e = !0, Ke(n, i);
        } finally{
            if (ye = !1, !e && !Y && f) {
                const i = Math.max(0, f.get_reference_orbit_len()), r = (d === 0 || R) && f.get_approximation_mode() !== 0;
                (n !== h || i < G || J || r) && C(h);
            }
        }
    }
    Re.onmessage = (n)=>{
        const e = n.data;
        try {
            switch(e.type){
                case "reset":
                    Y || Io(e);
                    break;
                case "updateView":
                    if (f && e.jobId === h) {
                        f.origin(e.cx, e.cy), f.scale(e.scale), f.angle(e.angle), e.viewportAspect !== void 0 && f.set_viewport_aspect(e.viewportAspect), G = e.maxIterations, J = !0;
                        const i = f.get_approximation_mode();
                        if (i === 3 || i === 4 || i === 5) {
                            const r = f.current_log2_c_max();
                            if (!Number.isFinite(_e) || r > _e || r < _e - 2) {
                                _e = r;
                                const a = i === 5 && f.get_dynamic_block_validity() && f.get_incremental_reference_table(), s = ho("viewport-update");
                                (!a || s) && (R = !0);
                            }
                        }
                        C(e.jobId);
                    }
                    break;
                case "setApproximationMode":
                    e.jobId === h && (Ye(e.approximationMode), f?.get_incremental_reference_table() && (f.set_incremental_reference_table(!1), f.set_incremental_reference_table(!0)), d = 0, R = !1, I = "", F = "epoch-reset", p = e.tableGeneration, C(e.jobId));
                    break;
                case "setBlaEpsilon":
                    f && e.jobId === h && (f.set_bla_epsilon(e.blaEpsilon), d = 0, R = !1, I = "", F = "epoch-reset", p = e.tableGeneration, C(e.jobId));
                    break;
                case "setGateEmission":
                    f && e.jobId === h && (f.set_gate_emission(e.on), d = 0, R = !1, I = "", p = e.tableGeneration, C(e.jobId));
                    break;
                case "setDynamicBlockValidity":
                    f && e.jobId === h && (f.set_dynamic_block_validity(e.on), d = 0, R = !1, I = "", F = "epoch-reset", p = e.tableGeneration, C(e.jobId));
                    break;
                case "setIncrementalReferenceTable":
                    f && e.jobId === h && (f.set_incremental_reference_table(e.on), d = 0, R = !1, I = "", F = "epoch-reset", p = e.tableGeneration, C(e.jobId));
                    break;
                case "setMaxBlaSkip":
                    f && e.jobId === h && (f.set_max_bla_skip(e.maxBlaSkip), d = 0, R = !1, I = "", F = "epoch-reset", p = e.tableGeneration, C(e.jobId));
                    break;
                case "findMinibrot":
                    if (f && e.jobId === h) {
                        const i = e.fill !== void 0, r = i ? f.find_minibrot_framed(e.maxIter, e.radiusFactor, e.fill) : f.find_minibrot(e.maxIter, e.radiusFactor), a = r[0];
                        S({
                            type: "minibrotFound",
                            jobId: e.jobId,
                            status: a,
                            cx: a === "ok" ? r[1] : null,
                            cy: a === "ok" ? r[2] : null,
                            period: a === "ok" ? Number(r[3]) : a === "nonewton" || a === "nosize" ? Number(r[1]) : null,
                            scale: a === "ok" && i ? r[4] : null
                        });
                    }
                    break;
                case "dispose":
                    Y = !0, f?.free(), f = void 0, Re.close();
                    break;
            }
        } catch (i) {
            Ke("jobId" in e ? e.jobId : h, i);
        }
    };
    S({
        type: "ready"
    });
})();
